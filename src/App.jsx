        ...
        {!sent ? (
          <>
            <div style={{display:"flex",alignItems:"center",gap:11,marginBottom:18}}>
              ...
              <div>
                <div style={{fontSize:11,fontWeight:900,color:"#fff",letterSpacing:"-0.01em"}}>Contact Us</div>
            ...
            <div style={{background:C.accentSoft,border:`1px solid rgba(121,186,236,0.22)`,borderRadius:8,padding:"9px 12px",marginBottom:16,display:"flex",alignItems:"center",justifyContent:"space-between",gap:10}}>
              <div>
                <div style={{fontSize:11,color:C.muted,marginBottom:2,letterSpacing:"0.05em"}}>OUR EMAIL</div>
                <div style={{fontSize:11,fontWeight:700,color:C.blue}}>{CONTACT_EMAIL}</div>
              </div>
              <button
                onClick={()=>navigator.clipboard.writeText(CONTACT_EMAIL)}
                style={{padding:"5px 10px",borderRadius:6,background:"transparent",border:`1px solid rgba(121,186,236,0.3)`,color:C.blue,fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit",flexShrink:0}}>
                Copy
              </button>
            </div>
            ...
            <div style={{marginBottom:11}}>
              <label style={{fontSize:11,letterSpacing:"0.08em",color:C.muted,display:"block",marginBottom:5,textTransform:"uppercase"}}>Subject</label>
              <input value={subject} onChange={e=>setSubject(e.target.value)} placeholder="e.g. Billing question, Feature request..."
                style={{width:"100%",background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,padding:"10px 12px",color:C.text,fontSize:11,fontFamily:"inherit"}}
            ...
            <div style={{marginBottom:16}}>
              ...
              <textarea value={message} onChange={e=>setMessage(e.target.value)} rows={4} placeholder="Tell us what's on your mind..."
                style={{width:"100%",background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,padding:"10px 12px",color:C.text,fontSize:11,lineHeight:1.6,resize:"none",fontFamily:"inherit"}}
                onFocus={e=>e.target.style.borderColor=C.blue} onBlur={e=>e.target.style.borderColor=C.border}/>
            </div>
            ...
            <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:18}}>
              {["Billing","Bug report","Feature request","General question"].map(t=>(
                <button key={t} onClick={()=>setSubject(t)}
                  style={{padding:"4px 10px",borderRadius:20,border:`1px solid ${C.border}`,background:subject===t?C.accentSoft:"transparent",color:subject===t?C.blue:C.muted,fontSize:11,fontWeight:600,cursor:"pointer",fontFamily:"inherit",transition:"all 0.15s"}}>
                  {t}
                </button>
              ))}
            </div>
            ...
            <button onClick={handleSend}
              style={{width:"100%",padding:"12px",borderRadius:8,border:"none",background:`linear-gradient(135deg,${C.blue},${C.accent})`,color:"#000",fontSize:11,fontWeight:800,cursor:"pointer",fontFamily:"inherit",boxShadow:`0 4px 20px ${C.blueGlow}`,marginBottom:10}}>
              Open Email App →
            </button>
          ...
          <div style={{textAlign:"center",padding:"16px 0"}}>
            ...
            <button onClick={onClose}
              style={{width:"100%",padding:"11px",borderRadius:8,background:"transparent",border:`1px solid ${C.border}`,color:C.muted,fontSize:11,cursor:"pointer",fontFamily:"inherit"}}>
              Close
            </button>
          </div>
        )}
...
function ImageInput({onImage, imageData, onClear}) {
  ...
  if (imageData) return (
    <div style={{marginBottom:12,position:"relative",display:"inline-block"}}>
      ...
      <button onClick={onClear}
        style={{position:"absolute",top:6,right:6,width:24,height:24,borderRadius:"50%",background:"rgba(0,0,0,0.7)",border:"none",color:"#fff",fontSize:13,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",lineHeight:1}}>
        ✕
      </button>
      <div style={{fontSize:12,color:C.muted,marginTop:4}}>📎 Image attached — AI will read it</div>
    </div>
  );
  ...
  return (
    <div style={{display:"flex",gap:7,marginBottom:12}}>
      ...
      <input ref={camRef}  type="file" accept="image/*" capture="environment" onChange={handleCam} style={{display:"none"}}/>
      <button onClick={()=>fileRef.current?.click()}
        style={{padding:"7px 12px",borderRadius:7,background:"transparent",border:`1px solid ${C.border}`,color:C.muted,fontSize:12,cursor:"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",gap:5,transition:"all 0.15s"}}
...
// ─── Save result as downloadable image ───────────────────────────────────────
function SaveAsImageBtn({text, title}) {
  ...
  const save = () => {
    ...
    try {
      ...
      ctx0.font = "16px 'Cabinet Grotesk', system-ui, sans-serif";
      const words = text.split(" ");
      const lines = [];
      let cur = "";
      for (const w of words) {
        const test = cur ? cur + " " + w : w;
        if (ctx0.measureText(test).width > maxW && cur) {
          lines.push(cur);
          cur = w;
        } else {
          cur = test;
      ...
      const headerH = 80;
      const footerH = 48;
      const height  = headerH + lines.length * lineH + padding + footerH;
      ...
      ctx.font = "bold 18px 'Cabinet Grotesk', system-ui, sans-serif";
      ctx.fillText("👻 GhostwriterMe", padding, 36);

      // Title tag
      if (title) {
        ctx.fillStyle = "#79BAEC";
        ctx.font = "12px 'Cabinet Grotesk', system-ui, sans-serif";
        ctx.fillText(title.toUpperCase(), padding, 58);
      }
      ...
      ctx.lineWidth = 1;
      ctx.beginPath();
      ctx.moveTo(padding, headerH - 8);
      ctx.lineTo(width - padding, headerH - 8);
      ctx.stroke();

      // Body text
      ctx.fillStyle = "#ddeeff";
      ctx.font = "16px 'Cabinet Grotesk', system-ui, sans-serif";
      lines.forEach((line, i) => {
        ctx.fillText(line, padding, headerH + i * lineH + 20);
      });

      // Footer
      ctx.fillStyle = "#3d5a75";
      ctx.font = "12px system-ui, sans-serif";
      ctx.fillText("ghostwriterme.com", padding, height - 18);
      ctx.textAlign = "right";
      ctx.fillText(new Date().toLocaleDateString("en-GB", {day:"2-digit",month:"short",year:"numeric"}), width - padding, height - 18);

      // Download
      const link = document.createElement("a");
      link.download = "ghostwriterme-result.png";
      link.href = canvas.toDataURL("image/png");
      link.click();
    } catch(e) {
      alert("Could not save image. Try the Copy button instead.");
    }
    setSaving(false);
  };
  ...
  return (
    <button onClick={save} disabled={saving}
      style={{padding:"6px 13px",borderRadius:6,background:"transparent",border:`1px solid ${C.border}`,color:C.muted,fontSize:12,cursor:"pointer",fontFamily:"inherit",transition:"all 0.2s",display:"flex",alignItems:"center",gap:5}}
      onMouseEnter={e=>{e.currentTarget.style.borderColor=C.blue;e.currentTarget.style.color=C.blue;}}
      onMouseLeave={e=>{e.currentTarget.style.borderColor=C.border;e.currentTarget.style.color=C.muted;}}>
...
function MicBtn({onResult, sm}) {
  ...
  return (
    <button onClick={toggle}
      style={{width:sz,height:sz,borderRadius:"50%",border:`1.5px solid ${active?C.blue:C.border}`,background:active?C.accentSoft:"transparent",color:active?C.blue:C.muted,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,flexShrink:0,transition:"all 0.2s",animation:active?"micPulse 1.5s infinite":"none"}}>
      {active?"⏹":"🎤"}
    </button>
  );
}
    ...
    <div style={{marginBottom:12}}>
      ...
      <div style={{position:"relative",display:"flex",alignItems:"center",gap:6}}>
        <div style={{position:"relative",flex:1}}>
          {icoL && <span style={{position:"absolute",left:11,top:"50%",transform:"translateY(-50%)",fontSize:11,pointerEvents:"none"}}>{icoL}</span>}
          <input type={type} placeholder={placeholder} value={value} onChange={onChange}
            onFocus={()=>setF(true)} onBlur={()=>setF(false)}
            style={{width:"100%",background:C.surface,border:`1px solid ${error?C.red:f?C.blue:C.border}`,borderRadius:8,padding:`10px ${icoR?40:12}px 10px ${icoL?38:12}px`,color:C.text,fontSize:11,transition:"border-color 0.2s"}}/>
          {icoR && <span onClick={onIcoR} style={{position:"absolute",right:11,top:"50%",transform:"translateY(-50%)",cursor:"pointer",fontSize:11}}>{icoR}</span>}
        </div>
    ...
    <div style={{marginBottom:12}}>
      ...
      <div style={{position:"relative"}}>
        <textarea value={value} onChange={onChange} rows={rows} placeholder={placeholder}
          onFocus={()=>setF(true)} onBlur={()=>setF(false)}
          style={{width:"100%",background:C.surface,border:`1px solid ${f?C.blue:C.border}`,borderRadius:8,padding:"11px 13px",color:C.text,fontSize:11,lineHeight:1.7,resize:"vertical",fontFamily:"inherit",transition:"border-color 0.2s"}}/>
    ...
    <div style={{marginBottom:0}}>
      ...
      <div style={{position:"relative"}}>
        <select value={value} onChange={e=>onChange(e.target.value)}
          style={{width:"100%",background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,padding:"10px 32px 10px 12px",color:C.text,fontSize:11,fontFamily:"inherit",cursor:"pointer"}}>
          {options.map(o=><option key={o.value??o} value={o.value??o}>{o.label??o}</option>)}
        </select>
        <span style={{position:"absolute",right:10,top:"50%",transform:"translateY(-50%)",pointerEvents:"none",color:C.muted,fontSize:11}}>▾</span>
      </div>
    </div>
  );
}
...
const PriBtn = ({children,onClick,loading,disabled,fullWidth=true,variant="blue"}) => {
  ...
  return (
    <button onClick={onClick} disabled={loading||disabled}
      style={{width:fullWidth?"100%":"auto",padding:"12px 20px",borderRadius:8,border:"none",background:bg,color:loading||disabled?C.muted:"#000",fontSize:11,fontWeight:800,cursor:loading||disabled?"not-allowed":"pointer",transition:"all 0.2s",display:"flex",alignItems:"center",justifyContent:"center",gap:8,boxShadow:loading||disabled?"none":variant==="violet"?"0 4px 20px rgba(155,127,232,0.3)":`0 4px 20px ${C.blueGlow}`,fontFamily:"inherit",letterSpacing:"0.01em"}}>
...
const SecBtn = ({children,onClick}) => (
  <button onClick={onClick}
    style={{width:"100%",padding:"11px",borderRadius:8,background:"transparent",border:`1px solid ${C.border}`,color:C.muted,fontSize:11,fontWeight:600,cursor:"pointer",transition:"all 0.2s",fontFamily:"inherit"}}
  ...
  return (
    <div style={{position:"fixed",inset:0,zIndex:500,background:C.bg,display:"flex",flexDirection:"column",animation:"fadeUp 0.2s ease",fontFamily:"'Cabinet Grotesk',sans-serif"}}>
      <div style={{background:"rgba(0,0,0,0.98)",borderBottom:`1px solid ${C.border}`,padding:"13px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
        <div style={{fontSize:11,fontWeight:800,color:"#fff"}}>Terms & Conditions</div>
        <button onClick={onClose} style={{width:30,height:30,borderRadius:"50%",background:C.surface,border:`1px solid ${C.border}`,color:C.muted,fontSize:11,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>✕</button>
      </div>
      <div style={{flex:1,overflowY:"auto",padding:"20px 16px 48px",maxWidth:620,width:"100%",margin:"0 auto"}}>
        {TERMS_CONTENT.map((s,i)=>(
          <div key={i} style={{marginBottom:20}}>
            <div style={{fontSize:11,fontWeight:700,color:C.blue,marginBottom:5}}>{s.h}</div>
            <div style={{fontSize:11,color:C.muted,lineHeight:1.75}}>{s.b}</div>
      ...
      <div style={{padding:"13px 16px",borderTop:`1px solid ${C.border}`,background:"rgba(0,0,0,0.98)"}}>
        <button onClick={onClose} style={{width:"100%",maxWidth:460,margin:"0 auto",display:"block",padding:"12px",borderRadius:8,background:`linear-gradient(135deg,${C.blue},${C.accent})`,color:"#000",fontSize:11,fontWeight:800,cursor:"pointer",border:"none",fontFamily:"inherit"}}>Got it — Close ✓</button>
      </div>
    </div>
  );
}
...
function SafetyScreen({onAccept}) {
  ...
  const CheckRow = ({checked,set,children}) => (
    <div onClick={set} style={{display:"flex",alignItems:"flex-start",gap:10,padding:"12px",background:checked?C.accentSoft:C.surface,border:`1px solid ${checked?C.blue:C.border}`,borderRadius:9,cursor:"pointer",transition:"all 0.15s",marginBottom:8}}>
      ...
      <div style={{fontSize:11,color:checked?C.text:C.muted,lineHeight:1.6,transition:"color 0.15s"}}>{children}</div>
    </div>
  );
  return (
    <div style={{minHeight:"100vh",background:C.bg,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"24px 16px",fontFamily:"'Cabinet Grotesk',sans-serif"}}>
      <div style={{width:"100%",maxWidth:420,animation:"fadeUp 0.4s ease"}}>
        <div style={{textAlign:"center",marginBottom:24}}>
          ...
          <div style={{fontSize:11,fontWeight:900,color:"#fff",letterSpacing:"-0.01em",marginBottom:6}}>Before You Begin</div>
          <div style={{fontSize:11,color:C.muted,lineHeight:1.6}}>Read and accept all three conditions to continue.</div>
        </div>
        ...
        <button onClick={onAccept} disabled={!all}
          style={{width:"100%",padding:"13px",borderRadius:8,border:"none",background:all?`linear-gradient(135deg,${C.blue},${C.accent})`:"#0c1220",color:all?"#000":C.muted,fontSize:11,fontWeight:800,cursor:all?"pointer":"not-allowed",transition:"all 0.3s",fontFamily:"inherit",boxShadow:all?`0 4px 20px ${C.blueGlow}`:"none"}}>
    ...
    <>
      ...
      <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"24px 16px",background:C.bg,fontFamily:"'Cabinet Grotesk',sans-serif"}}>
        <div style={{textAlign:"center",marginBottom:26,animation:"fadeUp 0.4s ease"}}>
          ...
          <div style={{fontSize:11,color:C.muted,letterSpacing:"0.18em",marginTop:5}}>AI WRITING SUITE</div>
        </div>
        <div style={{width:"100%",maxWidth:370,background:C.card,border:`1px solid ${C.border}`,borderRadius:14,padding:"22px 18px",animation:"fadeUp 0.4s 0.08s ease both",boxShadow:"0 20px 50px rgba(0,0,0,0.7)"}}>
          <div style={{display:"flex",background:C.surface,borderRadius:7,padding:3,marginBottom:20}}>
            {["signin","signup"].map(t=>(
              <button key={t} onClick={()=>{setTab(t);setShowEmail(false);setErrs({});setAgreed(false);setAge("");}}
                style={{flex:1,padding:"7px",borderRadius:5,border:"none",background:tab===t?C.blue:"transparent",color:tab===t?"#000":C.muted,fontSize:11,fontWeight:800,cursor:"pointer",transition:"all 0.2s",fontFamily:"inherit"}}>
          ...
          {!showEmail?(
            <>
              <div style={{fontSize:11,fontWeight:800,color:"#fff",marginBottom:3}}>{tab==="signin"?"Welcome back 👋":"Join GhostwriterMe"}</div>
              <div style={{fontSize:11,color:C.muted,marginBottom:18,lineHeight:1.5}}>{tab==="signin"?"Access your AI writing suite.":"Free forever. No card needed."}</div>
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                {SOCIAL_PROVIDERS.map(s=>(
                  <button key={s.id} onClick={()=>handleSocial(s.id)} disabled={!!loading}
                    style={{display:"flex",alignItems:"center",justifyContent:"center",gap:10,padding:"15px",borderRadius:8,border:s.border||"none",background:s.bg,color:s.color,fontSize:11,fontWeight:700,cursor:loading?"not-allowed":"pointer",transition:"opacity 0.2s,transform 0.15s",opacity:loading&&loading!==s.id?0.4:1,fontFamily:"inherit"}}
            ...
            <div style={{animation:"slideIn 0.25s ease"}}>
                ...
                <>
                  ...
                  <div style={{marginBottom:12}}>
                    ...
                    <input type="number" min="1" max="120" placeholder="e.g. 20" value={age} onChange={e=>setAge(e.target.value)}
                      style={{width:"100%",background:C.surface,border:`1px solid ${errs.age?C.red:C.border}`,borderRadius:8,padding:"10px 12px",color:C.text,fontSize:11}}/>
                    {errs.age&&<div style={{fontSize:11,color:C.red,marginTop:3}}>{errs.age}</div>}
                    <div style={{fontSize:11,color:C.muted,marginTop:3}}>Minimum age: 13</div>
                  </div>
                </>
              )}
              ...
              {tab==="signup"&&(
                <div style={{marginBottom:12}}>
                  ...
                  {errs.terms&&<div style={{fontSize:11,color:C.red,marginTop:3}}>{errs.terms}</div>}
                </div>
              )}
    ...
    <div style={{minHeight:"100vh",background:C.bg,padding:"24px 14px 80px",display:"flex",flexDirection:"column",alignItems:"center",fontFamily:"'Cabinet Grotesk',sans-serif"}}>
      <div style={{width:"100%",maxWidth:440}}>
        ...
        <div style={{
          ...
          {tab!=="free"&&(
            <div style={{position:"absolute",top:-1,right:14,background:tab==="student"?"linear-gradient(135deg,#9b7fe8,#c4b5fd)":`linear-gradient(135deg,${C.blue},${C.accent})`,color:"#000",fontSize:11,fontWeight:900,letterSpacing:"0.08em",padding:"3px 10px",borderRadius:"0 0 6px 6px"}}>
          ...
          <div style={{display:"flex",alignItems:"baseline",gap:4,marginBottom:2}}>
            ...
            <span style={{fontSize:11,color:C.muted}}>{price.per}</span>
          </div>
          {price.sub&&<div style={{fontSize:11,color:(proBill==="yearly"||stuBill==="yearly")?C.green:C.muted,marginBottom:14}}>{price.sub}</div>}
          ...
          <ul style={{listStyle:"none",marginBottom:16,display:"flex",flexDirection:"column",gap:5,maxHeight:270,overflowY:"auto"}}>
            {features.map((f,i)=>{
              ...
              return (
                <li key={f} style={{fontSize:11,color:isStuEx?C.accent:isProEx||isProBas?C.text:C.muted,display:"flex",alignItems:"flex-start",gap:6}}>
          ...
          {tab!=="free"&&(
            <div style={{background:tab==="student"?C.violetSoft:C.accentSoft,border:`1px solid ${tab==="student"?"rgba(155,127,232,0.2)":"rgba(121,186,236,0.2)"}`,borderRadius:8,padding:"9px 11px",marginBottom:13,display:"flex",alignItems:"center",gap:8}}>
              ...
              <div><div style={{fontSize:11,fontWeight:700,color:"#fff"}}>3-day free trial</div><div style={{fontSize:11,color:C.muted}}>No charge until day 4 · Cancel anytime</div></div>
            </div>
          )}
        ...
        <div style={{display:"flex",justifyContent:"center",gap:5,flexWrap:"wrap",animation:"fadeUp 0.4s 0.18s ease both"}}>
          {tabs.filter(t=>t.id!==tab).map(t=>(
            <button key={t.id} onClick={()=>setTab(t.id)}
              style={{padding:"4px 12px",borderRadius:20,border:`1px solid ${C.border}`,background:"transparent",color:C.muted,fontSize:11,cursor:"pointer",fontFamily:"inherit",transition:"all 0.15s"}}
  ...
  if(step==="success") return (
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",padding:"24px",background:C.bg,fontFamily:"'Cabinet Grotesk',sans-serif"}}>
      <div style={{textAlign:"center",maxWidth:320,animation:"fadeUp 0.5s ease"}}>
        ...
        <div style={{fontSize:11,color:C.muted,lineHeight:1.7,marginBottom:22}}>{isStudent?"Student plan activated!":"3-day free trial started."}<br/>All features unlocked. 🚀</div>
    ...
    <div style={{minHeight:"100vh",background:C.bg,padding:"24px 14px 80px",display:"flex",flexDirection:"column",alignItems:"center",fontFamily:"'Cabinet Grotesk',sans-serif"}}>
      <div style={{width:"100%",maxWidth:420}}>
        <div style={{marginBottom:18}}>
          <div style={{fontSize:11,fontWeight:900,color:"#fff",letterSpacing:"-0.01em"}}>Payment</div>
          <div style={{fontSize:11,color:C.muted}}>Secure checkout · SSL encrypted · Cancel anytime</div>
        </div>
        <Card style={{marginBottom:16}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div>
              <div style={{fontSize:11,fontWeight:800,color:"#fff"}}>GhostwriterMe {isStudent?"Student":"Pro"}</div>
              <div style={{fontSize:11,color:C.muted,marginTop:1}}>{billing} · after 3-day trial</div>
            </div>
            <div style={{textAlign:"right"}}>
              <div style={{fontSize:11,fontWeight:900,color:planColor}}>{priceStr.split(" ")[0]}</div>
              <div style={{fontSize:11,color:C.green,marginTop:1}}>Today: $0.00 ✓</div>
            </div>
          </div>
        </Card>
        <div style={{marginBottom:16}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {METHODS.map(m=>(
              <button key={m.id} onClick={()=>setMethod(m.id)}
                ...
                <div style={{width:38,height:38,borderRadius:8,flexShrink:0,background:m.color,display:"flex",alignItems:"center",justifyContent:"center",fontSize:11}}>{m.icon}</div>
                <div style={{flex:1}}><div style={{fontSize:11,fontWeight:700,color:"#fff"}}>{m.label}</div><div style={{fontSize:11,color:C.muted,marginTop:1}}>{m.desc}</div></div>
                {method===m.id&&<div style={{width:18,height:18,borderRadius:"50%",background:planColor,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><span style={{fontSize:11,color:"#000",fontWeight:900}}>✓</span></div>}
              </button>
            ))}
          </div>
        </div>
        {method==="card"&&(
          <Card style={{marginBottom:14,animation:"slideIn 0.3s ease"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:11}}>
              ...
              <div style={{display:"flex",gap:4}}>{["VISA","MC"].map(b=><div key={b} style={{background:brand(card.number)===b?"#fff":"transparent",border:`1px solid ${brand(card.number)===b?"#fff":C.border}`,borderRadius:3,padding:"1px 6px",fontSize:11,fontWeight:900,color:brand(card.number)===b?"#000":C.muted}}>{b}</div>)}</div>
            </div>
            ...
            <div style={{fontSize:11,color:C.muted}}>🔒 Processed by Stripe. We never store card data.</div>
          </Card>
        )}
        {method==="applepay"&&<Card style={{marginBottom:14,textAlign:"center",animation:"slideIn 0.3s ease"}}><div style={{fontSize:30,marginBottom:7}}>🍎</div><div style={{fontSize:11,fontWeight:700,color:"#fff",marginBottom:3}}>Apple Pay</div><div style={{fontSize:11,color:C.muted}}>Authenticate with Face ID or Touch ID.</div></Card>}
        ...
        {method&&(
          <div style={{animation:"fadeUp 0.3s ease"}}>
            ...
            <div style={{textAlign:"center",fontSize:11,color:C.muted,marginTop:7}}>🔒 Secure · No charge today · Cancel anytime</div>
          </div>
        )}
      </div>
    </div>
      ...
      <div style={{display:"flex",gap:5,marginBottom:14,overflowX:"auto",paddingBottom:3}}>
        {["all",...Object.keys(ML)].map(m=>(
          <button key={m} onClick={()=>setFilter(m)}
            style={{flexShrink:0,padding:"5px 10px",borderRadius:20,border:`1px solid ${filter===m?C.blue:C.border}`,background:filter===m?C.accentSoft:"transparent",color:filter===m?C.blue:C.muted,fontSize:11,fontWeight:600,cursor:"pointer",fontFamily:"inherit",transition:"all 0.15s"}}>
            {m==="all"?"All":(MI[m]||"")+" "+(ML[m]||m)}
          </button>
        ))}
      </div>
      <div style={{fontSize:11,color:C.muted,marginBottom:9}}>{filtered.length} item{filtered.length!==1?"s":""}</div>
      {filtered.map(item=>(
        <Card key={item.id} style={{marginBottom:8}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:exp===item.id?9:0}}>
            <div style={{display:"flex",alignItems:"center",gap:7,flex:1,minWidth:0}}>
              <span style={{fontSize:11,flexShrink:0}}>{MI[item.mode]||"📝"}</span>
              <div style={{minWidth:0}}>
                <div style={{fontSize:11,color:C.muted}}>{ML[item.mode]||item.mode} · {new Date(item.ts).toLocaleDateString("en-GB",{day:"2-digit",month:"short",hour:"2-digit",minute:"2-digit"})}</div>
                <div style={{fontSize:11,color:C.text,fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{item.title||item.output?.slice(0,55)||"Untitled"}</div>
              </div>
            </div>
            <button onClick={()=>setExp(exp===item.id?null:item.id)}
              style={{flexShrink:0,padding:"4px 8px",borderRadius:5,border:`1px solid ${C.border}`,background:"transparent",color:C.muted,fontSize:11,cursor:"pointer",fontFamily:"inherit",marginLeft:6}}>
              {exp===item.id?"Hide":"View"}
            </button>
          </div>
          {exp===item.id&&(
            <div style={{animation:"fadeUp 0.2s ease"}}>
              {item.input&&<div style={{fontSize:11,color:C.muted,background:C.surface,borderRadius:6,padding:"7px 10px",marginBottom:7,lineHeight:1.5}}><strong>Input:</strong> {item.input}</div>}
              <div style={{fontSize:11,lineHeight:1.8,color:C.text,whiteSpace:"pre-wrap",maxHeight:200,overflowY:"auto",background:C.surface,borderRadius:6,padding:"9px 11px"}}>{item.output}</div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}}>
        {TONES.map(t=>(
          <button key={t.id} onClick={()=>setTone(t.id)} style={{background:tone===t.id?C.accentSoft:C.surface,border:`1px solid ${tone===t.id?C.blue:C.border}`,borderRadius:8,padding:"9px 10px",cursor:"pointer",textAlign:"left",color:C.text,fontFamily:"inherit",transition:"all 0.15s"}}>
            <div style={{fontSize:11}}>{t.emoji}</div><div style={{fontSize:11,fontWeight:700,marginTop:3}}>{t.label}</div><div style={{fontSize:11,color:C.muted,marginTop:1}}>{t.desc}</div>
          </button>
        ))}
      </div>
        ...
        <div ref={ref} style={{marginTop:20,animation:"fadeUp 0.4s ease"}}>
          {replies.map((r,i)=>(
            <Card key={i} style={{marginTop:9}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}><span style={{fontSize:11,color:C.accent,textTransform:"uppercase",letterSpacing:"0.1em"}}>{r.vibe}</span><span style={{fontSize:11,color:C.muted}}>Option {r.option}</span></div>
              <div style={{fontSize:11,lineHeight:1.7,color:C.text}}>{r.text}</div>
              <div style={{display:"flex",gap:7,marginTop:11,flexWrap:"wrap"}}><CopyBtn text={r.text}/><ListenBtn text={r.text}/><SaveAsImageBtn text={r.text} title={"AI Reply"}/></div>
            </Card>
          ))}
        </div>
      )}
  ...
  return (
    <div>
      ...
      {res&&<div style={{marginTop:16,animation:"fadeUp 0.4s ease"}}>
        <Card style={{marginBottom:9}}><div style={{fontSize:11,color:C.muted,textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:5}}>Subject</div><div style={{fontSize:11,fontWeight:800,color:"#fff"}}>{res.subject}</div><div style={{display:"flex",gap:7,marginTop:11,flexWrap:"wrap"}}><CopyBtn text={res.subject}/><ListenBtn text={res.subject}/><SaveAsImageBtn text={res.subject} title={"Email Subject"}/></div></Card>
        <Card style={{marginBottom:9}}><div style={{fontSize:11,color:C.accent,textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:7}}>Body</div><div style={{fontSize:11,lineHeight:1.85,color:C.text,whiteSpace:"pre-wrap"}}>{res.body}</div><div style={{display:"flex",gap:7,marginTop:11,flexWrap:"wrap"}}><CopyBtn text={res.body}/><ListenBtn text={res.body}/><SaveAsImageBtn text={res.body} title={"Email"}/></div></Card>
        {res.tip&&<div style={{background:"rgba(245,200,66,0.06)",border:"1px solid rgba(245,200,66,0.15)",borderRadius:8,padding:"10px 12px",display:"flex",gap:8}}><span style={{fontSize:11}}>💡</span><div style={{fontSize:11,color:C.yellow,lineHeight:1.6}}>{res.tip}</div></div>}
      </div>}
    </div>
  );
}
    ...
    <div>
      ...
      <div style={{marginBottom:12}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
          <div style={{fontSize:11,letterSpacing:"0.1em",color:C.muted,textTransform:"uppercase"}}>Sources to Cite</div>
          <button onClick={addC} style={{background:"transparent",border:`1px solid ${C.border}`,borderRadius:5,padding:"3px 9px",color:C.blue,fontSize:11,cursor:"pointer",fontFamily:"inherit"}}>+ Add</button>
        </div>
        {cites.map((c,i)=>(
          <div key={i} style={{display:"flex",gap:6,marginBottom:7,alignItems:"center"}}>
            <select value={c.type} onChange={e=>updC(i,"type",e.target.value)} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:6,padding:"8px 6px",color:C.text,fontSize:11,fontFamily:"inherit",width:76,flexShrink:0}}>
            ...
            <input value={c.value} onChange={e=>updC(i,"value",e.target.value)} placeholder={c.type==="url"?"https://...":"Author, Title, Year..."}
              ...
              onFocus={e=>e.target.style.borderColor=C.blue} onBlur={e=>e.target.style.borderColor=C.border}/>
            {cites.length>1&&<button onClick={()=>remC(i)} style={{background:"none",border:"none",color:C.muted,cursor:"pointer",fontSize:11,flexShrink:0}}>✕</button>}
          </div>
        ))}
      </div>
      <ImageInput onImage={(d,t)=>{setImgData(d);setImgType(t);}} imageData={imgData} onClear={()=>{setImgData(null);setImgType(null);}}/>
  ...
  return (
    <div>
      ...
      <div style={{background:C.violetSoft,border:"1px solid rgba(155,127,232,0.28)",borderRadius:8,padding:"11px 13px",marginBottom:14,display:"flex",gap:9}}>
        <span style={{fontSize:11,flexShrink:0}}>🧠</span>
        <div>
          <div style={{fontSize:11,fontWeight:800,color:C.violet,marginBottom:2}}>Humanize My Writing — Student Exclusive</div>
      ...
      {/* CEFR Level */}
      <div style={{marginBottom:13}}>
        <div style={{fontSize:11,letterSpacing:"0.1em",color:C.muted,textTransform:"uppercase",marginBottom:7}}>Your English Level (CEFR)</div>
        <div style={{display:"flex",gap:5}}>
          {LEVELS.map(l=>(
      ...
      {/* Purpose */}
      <div style={{marginBottom:13}}>
        <div style={{fontSize:11,letterSpacing:"0.1em",color:C.muted,textTransform:"uppercase",marginBottom:8}}>Writing Purpose</div>
      ...
      <div style={{marginBottom:14}}>
        <div style={{fontSize:11,letterSpacing:"0.1em",color:C.muted,textTransform:"uppercase",marginBottom:8}}>Transformation Intensity</div>
        {INTENSITIES.map(iv=>(
          <div key={iv.id} onClick={()=>setIntensity(iv.id)}
            ...
            <div>
              <div style={{fontSize:11,fontWeight:700,color:intensity===iv.id?C.violet:C.text}}>{iv.label}</div>
              <div style={{fontSize:11,color:C.muted,marginTop:1}}>{iv.desc}</div>
            </div>
      ...
      {isLoading && (
        <div style={{marginTop:10,display:"flex",gap:6,alignItems:"center"}}>
          ...
          <div style={{fontSize:11,color:C.violet,flexShrink:0,fontFamily:"'Cabinet Grotesk',sans-serif"}}>
        ...
        <div style={{marginTop:16,animation:"fadeUp 0.4s ease"}}>
          ...
          {res.changes?.length>0 && (
            <Card style={{marginBottom:10,borderColor:"rgba(155,127,232,0.3)"}}>
              <div style={{fontSize:11,color:C.violet,textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:9}}>What Changed</div>
              {res.changes.map((c,i)=>(
                <div key={i} style={{display:"flex",gap:8,paddingBottom:i<res.changes.length-1?8:0,marginBottom:i<res.changes.length-1?8:0,borderBottom:i<res.changes.length-1?`1px solid ${C.border}`:"none"}}>
                  ...
                  <div>
                    <div style={{fontSize:11,fontWeight:700,color:C.text}}>{c.what}</div>
                    <div style={{fontSize:11,color:C.muted,marginTop:1}}>{c.why}</div>
                  </div>
                </div>
              ))}
            </Card>
          ...
          {view==="output" && (
            <Card glow glowColor={C.violet}>
              <div style={{fontSize:11,color:C.violet,textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:8}}>Humanized Output · 2-Pass Reviewed</div>
              <div style={{fontSize:11,lineHeight:1.9,color:C.text,whiteSpace:"pre-wrap"}}>{res.humanized}</div>
              <div style={{display:"flex",gap:7,marginTop:11,flexWrap:"wrap"}}><CopyBtn text={res.humanized}/><ListenBtn text={res.humanized}/><SaveAsImageBtn text={res.humanized} title={"Humanized Writing"}/></div>
            </Card>
          )}
          ...
          {view==="compare" && (
            <div>
              ...
              <div style={{marginBottom:10}}>
                <div style={{fontSize:11,color:C.red,textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:6,fontFamily:"'Cabinet Grotesk',sans-serif"}}>Before</div>
                <div style={{background:"rgba(240,107,107,0.05)",border:"1px solid rgba(240,107,107,0.2)",borderRadius:8,padding:"12px 14px",fontSize:11,lineHeight:1.9,color:C.text,whiteSpace:"pre-wrap"}}>
                  {text}
                </div>
              </div>
              ...
              <div style={{marginBottom:10}}>
                <div style={{fontSize:11,color:C.violet,textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:6,fontFamily:"'Cabinet Grotesk',sans-serif"}}>After</div>
                <div style={{background:"rgba(155,127,232,0.05)",border:"1px solid rgba(155,127,232,0.25)",borderRadius:8,padding:"12px 14px",fontSize:11,lineHeight:1.9,color:C.text,whiteSpace:"pre-wrap"}}>
              ...
              <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:10}}>
                {[
                  ...
                  <div key={s.label} style={{flex:1,minWidth:70,background:C.surface,border:`1px solid ${C.border}`,borderRadius:7,padding:"8px 10px",textAlign:"center"}}>
                    <div style={{fontSize:11,fontWeight:900,color:C.violet,fontFamily:"'Cabinet Grotesk',sans-serif"}}>{s.val}</div>
    ...
    <div style={{position:"fixed",inset:0,zIndex:200,display:"flex",alignItems:"flex-end",justifyContent:"center",background:"rgba(0,0,0,0.8)",backdropFilter:"blur(6px)",animation:"fadeUp 0.2s ease"}} onClick={e=>{if(e.target===e.currentTarget)onClose();}}>
      <div style={{width:"100%",maxWidth:500,background:C.card,border:`1px solid ${isStudent?"rgba(155,127,232,0.4)":C.border}`,borderRadius:"14px 14px 0 0",padding:"20px 16px 28px",animation:"slideUpModal 0.3s ease",maxHeight:"92vh",overflowY:"auto",fontFamily:"'Cabinet Grotesk',sans-serif"}}>
        ...
        <div style={{background:C.surface,borderRadius:9,padding:"11px 13px",marginBottom:14}}>
          {h.perks.map(p=><div key={p} style={{display:"flex",gap:8,fontSize:11,color:C.text,padding:"3px 0"}}><span style={{color:isStudent?C.violet:C.green,flexShrink:0}}>✓</span>{p}</div>)}
        </div>
        {!isStudent&&(
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:11}}>
            <div>
              <div style={{fontSize:11,fontWeight:900,color:"#fff",letterSpacing:"-0.02em"}}>
                ...
                <span style={{fontSize:11,color:C.muted,fontWeight:400}}>{isStudent?" / 2 months":"/month"}</span>
              </div>
              {!isStudent&&bill==="yearly"&&<div style={{fontSize:11,color:C.green,marginTop:1}}>Save $24/year</div>}
              {isStudent&&<div style={{fontSize:11,color:C.muted,marginTop:1}}>≈ $10/month · or $96/year</div>}
              {!isStudent&&<div style={{fontSize:11,color:C.muted,marginTop:1}}>billed in USD</div>}
            </div>
...
function AppShell({user, onStartTrial, onSignOut}) {
  ...
  const badgeFor = m => {
    ...
    if(m.access==="student") return <span style={{background:C.violetSoft,color:C.violet,fontSize:11,fontWeight:800,padding:"2px 6px",borderRadius:3,textTransform:"uppercase",flexShrink:0}}>🎓</span>;
    return <span style={{background:C.accentSoft,color:C.blue,fontSize:11,fontWeight:800,padding:"1px 5px",borderRadius:3,textTransform:"uppercase",flexShrink:0}}>PRO</span>;
  };
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'Cabinet Grotesk',sans-serif",color:C.text,paddingBottom:80}}>
      ...
      <div style={{position:"sticky",top:0,zIndex:50,background:"rgba(0,0,0,0.95)",backdropFilter:"blur(14px)",borderBottom:`1px solid ${C.border}`,padding:"10px 13px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <div>
          <div style={{fontSize:11,fontWeight:900,color:"#fff",letterSpacing:"-0.01em",lineHeight:1}}>👻 GhostwriterMe</div>
          <div style={{fontSize:11,color:C.muted,letterSpacing:"0.15em"}}>AI WRITING SUITE</div>
        </div>
        <div style={{display:"flex",alignItems:"center",gap:7}}>
          {/* Contact Us button */}
          <button onClick={()=>setShowContact(true)}
            style={{padding:"4px 10px",borderRadius:20,border:`1px solid ${C.border}`,background:"transparent",color:C.muted,fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit",transition:"all 0.15s",display:"flex",alignItems:"center",gap:4}}
            onMouseEnter={e=>{e.currentTarget.style.borderColor=C.blue;e.currentTarget.style.color=C.blue;}}
            onMouseLeave={e=>{e.currentTarget.style.borderColor=C.border;e.currentTarget.style.color=C.muted;}}>
            ✉️ Contact
          </button>
          ...
          {!isPro&&(
            <div style={{display:"flex",gap:5}}>
              ...
              <button onClick={()=>{setTPlan("student");setTMode("humanize");}} style={{background:C.violetSoft,border:"1px solid rgba(155,127,232,0.28)",borderRadius:20,padding:"3px 8px",fontSize:11,color:C.violet,fontWeight:800,cursor:"pointer",fontFamily:"inherit"}}>Student $20</button>
            </div>
          )}
      ...
      <div style={{display:"flex",borderBottom:`1px solid ${C.border}`,overflowX:"auto",scrollbarWidth:"none"}}>
        {MODES.map(m=>{
          ...
          const isHumanize = m.id==="humanize";
          const tabColor = isHumanize ? C.violet : planColor;
          return (
            <button key={m.id} onClick={()=>handleTab(m)}
              style={{flex:1,minWidth:54,background:isHumanize&&!isStudent?"rgba(155,127,232,0.05)":"transparent",border:"none",borderBottom:`2px solid ${isActive?tabColor:"transparent"}`,padding:"9px 2px 7px",color:isActive?tabColor:isHumanize?C.violet+"99":C.muted,fontFamily:"inherit",fontSize:11,cursor:"pointer",transition:"all 0.15s",display:"flex",flexDirection:"column",alignItems:"center",gap:2}}>
      ...
      <div style={{maxWidth:580,margin:"0 auto",padding:"15px 13px 0"}}>
        {isPro&&(
          <div style={{background:isStudent?C.violetSoft:C.accentSoft,border:`1px solid ${isStudent?"rgba(155,127,232,0.2)":"rgba(121,186,236,0.2)"}`,borderRadius:7,padding:"8px 12px",marginBottom:16,display:"flex",alignItems:"center",gap:8,fontSize:11,color:isStudent?C.violet:"#a8d4f5"}}>
        ...
        <div style={{marginTop:44,paddingTop:16,borderTop:`1px solid ${C.border}`,display:"flex",justifyContent:"space-between",flexWrap:"wrap",gap:6,fontSize:11,color:"#162030",alignItems:"center"}}>
          ...
          <button onClick={()=>setShowContact(true)}
            style={{background:"transparent",border:"none",color:C.muted,fontSize:11,cursor:"pointer",fontFamily:"inherit",padding:0,textDecoration:"underline"}}>
            Contact Us ✉️
          </button>
