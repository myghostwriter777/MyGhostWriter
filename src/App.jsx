...
// CHANGE 1: academic access "student" (was "pro+student")
const MODES = [
        ...
        {!sent?(
          <>
            <div style={{display:"flex",alignItems:"center",gap:11,marginBottom:18}}>
              <div style={{width:42,height:42,borderRadius:10,background:`linear-gradient(135deg,${C.blue},${C.accent})`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,flexShrink:0}}>✉️</div>
              <div><div style={{fontSize:11,fontWeight:900,color:"#fff"}}>Contact Us</div><div style={{fontSize:11,color:C.muted,marginTop:1}}>We typically reply within 24 hours</div></div>
            </div>
            <div style={{background:C.accentSoft,border:"1px solid rgba(121,186,236,0.22)",borderRadius:8,padding:"9px 12px",marginBottom:16,display:"flex",alignItems:"center",justifyContent:"space-between",gap:10}}>
              <div><div style={{fontSize:11,color:C.muted,marginBottom:2,letterSpacing:"0.05em"}}>OUR EMAIL</div><div style={{fontSize:11,fontWeight:700,color:C.blue}}>{CONTACT_EMAIL}</div></div>
              <button onClick={()=>navigator.clipboard.writeText(CONTACT_EMAIL)} style={{padding:"5px 10px",borderRadius:6,background:"transparent",border:"1px solid rgba(121,186,236,0.3)",color:C.blue,fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit",flexShrink:0}}>Copy</button>
            </div>
            <div style={{marginBottom:11}}><label style={{fontSize:11,letterSpacing:"0.08em",color:C.muted,display:"block",marginBottom:5,textTransform:"uppercase"}}>Subject</label><input value={subject} onChange={e=>setSubject(e.target.value)} placeholder="e.g. Billing question..." style={{width:"100%",background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,padding:"10px 12px",color:C.text,fontSize:11,fontFamily:"inherit"}} onFocus={e=>e.target.style.borderColor=C.blue} onBlur={e=>e.target.style.borderColor=C.border}/></div>
            <div style={{marginBottom:16}}><label style={{fontSize:11,letterSpacing:"0.08em",color:C.muted,display:"block",marginBottom:5,textTransform:"uppercase"}}>Message</label><textarea value={message} onChange={e=>setMessage(e.target.value)} rows={4} placeholder="Tell us what's on your mind..." style={{width:"100%",background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,padding:"10px 12px",color:C.text,fontSize:11,lineHeight:1.6,resize:"none",fontFamily:"inherit"}} onFocus={e=>e.target.style.borderColor=C.blue} onBlur={e=>e.target.style.borderColor=C.border}/></div>
            <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:18}}>
              {["Billing","Bug report","Feature request","General question"].map(t=>(
                <button key={t} onClick={()=>setSubject(t)} style={{padding:"4px 10px",borderRadius:20,border:`1px solid ${C.border}`,background:subject===t?C.accentSoft:"transparent",color:subject===t?C.blue:C.muted,fontSize:11,fontWeight:600,cursor:"pointer",fontFamily:"inherit",transition:"all 0.15s"}}>{t}</button>
              ))}
            </div>
            <button onClick={handleSend} style={{width:"100%",padding:"12px",borderRadius:8,border:"none",background:`linear-gradient(135deg,${C.blue},${C.accent})`,color:"#000",fontSize:11,fontWeight:800,cursor:"pointer",fontFamily:"inherit",boxShadow:`0 4px 20px ${C.blueGlow}`,marginBottom:10}}>Open Email App →</button>
            <div style={{textAlign:"center",fontSize:11,color:C.muted}}>This will open your default email app with the message pre-filled.</div>
          </>
        ):(
          <div style={{textAlign:"center",padding:"16px 0"}}>
            <div style={{fontSize:48,marginBottom:12}}>📬</div>
            <div style={{fontSize:11,fontWeight:900,color:"#fff",marginBottom:6}}>Email App Opened!</div>
            <div style={{fontSize:11,color:C.muted,lineHeight:1.7,marginBottom:20}}>Send the email from your mail app.<br/>We'll get back to you within 24 hours.</div>
            <div style={{background:C.accentSoft,border:"1px solid rgba(121,186,236,0.22)",borderRadius:8,padding:"10px 14px",marginBottom:18}}>
              ...
              <div style={{fontSize:11,fontWeight:700,color:C.blue}}>{CONTACT_EMAIL}</div>
            </div>
            <button onClick={onClose} style={{width:"100%",padding:"11px",borderRadius:8,background:"transparent",border:`1px solid ${C.border}`,color:C.muted,fontSize:11,cursor:"pointer",fontFamily:"inherit"}}>Close</button>
          </div>
        )}
      </div>
    </div>
...
function SocialIcon({type}){
  ...
  return <span style={{fontSize:11}}>✉️</span>;
}
...
const ErrBox=({msg})=><div style={{marginTop:10,padding:"10px 14px",background:"#1a0000",border:"1px solid #3a0808",borderRadius:8,fontSize:11,color:C.red}}>{msg}</div>;

function CopyBtn({text}){
  const [done,setDone]=useState(false);
  return <button onClick={()=>{navigator.clipboard.writeText(text);setDone(true);setTimeout(()=>setDone(false),2000);}} style={{padding:"6px 13px",borderRadius:6,background:done?"#0a2a18":"transparent",border:`1px solid ${done?C.green:C.border}`,color:done?C.green:C.muted,fontSize:11,cursor:"pointer",fontFamily:"inherit",transition:"all 0.2s"}}>{done?"✓ Copied":"Copy"}</button>;
}

function ListenBtn({text}){
  const [on,setOn]=useState(false);
  return <button onClick={()=>{if(on){stopSpeak();setOn(false);}else{speak(text);setOn(true);}}} style={{padding:"6px 12px",borderRadius:6,background:on?C.accentSoft:"transparent",border:`1px solid ${on?C.blue:C.border}`,color:on?C.blue:C.muted,fontSize:11,cursor:"pointer",fontFamily:"inherit",transition:"all 0.2s",display:"flex",alignItems:"center",gap:5}}>{on?"⏹ Stop":"🔊 Listen"}</button>;
}
...
function MicBtn({onResult,sm}){
  ...
  return <button onClick={toggle} style={{width:sz,height:sz,borderRadius:"50%",border:`1.5px solid ${active?C.blue:C.border}`,background:active?C.accentSoft:"transparent",color:active?C.blue:C.muted,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,flexShrink:0,transition:"all 0.2s",animation:active?"micPulse 1.5s infinite":"none"}}>{active?"⏹":"🎤"}</button>;
}

function FInput({label,type="text",placeholder,value,onChange,error,icoL,icoR,onIcoR,voice}){
  const [f,setF]=useState(false);
  return(<div style={{marginBottom:12}}>{label&&<label style={{fontSize:11,letterSpacing:"0.08em",color:C.muted,display:"block",marginBottom:5,textTransform:"uppercase"}}>{label}</label>}<div style={{position:"relative",display:"flex",alignItems:"center",gap:6}}><div style={{position:"relative",flex:1}}>{icoL&&<span style={{position:"absolute",left:11,top:"50%",transform:"translateY(-50%)",fontSize:11,pointerEvents:"none"}}>{icoL}</span>}<input type={type} placeholder={placeholder} value={value} onChange={onChange} onFocus={()=>setF(true)} onBlur={()=>setF(false)} style={{width:"100%",background:C.surface,border:`1px solid ${error?C.red:f?C.blue:C.border}`,borderRadius:8,padding:`10px ${icoR?40:12}px 10px ${icoL?38:12}px`,color:C.text,fontSize:11,transition:"border-color 0.2s"}}/>{icoR&&<span onClick={onIcoR} style={{position:"absolute",right:11,top:"50%",transform:"translateY(-50%)",cursor:"pointer",fontSize:11}}>{icoR}</span>}</div>{voice&&<MicBtn onResult={t=>onChange({target:{value:value+(value?" ":"")+t}})} sm/>}</div>{error&&<div style={{fontSize:11,color:C.red,marginTop:3}}>{error}</div>}</div>);
}

function FArea({label,placeholder,value,onChange,rows=4,hint,voice}){
  const [f,setF]=useState(false);
  return(<div style={{marginBottom:12}}>{label&&<label style={{fontSize:11,letterSpacing:"0.08em",color:C.muted,display:"block",marginBottom:5,textTransform:"uppercase"}}>{label}</label>}<div style={{position:"relative"}}><textarea value={value} onChange={onChange} rows={rows} placeholder={placeholder} onFocus={()=>setF(true)} onBlur={()=>setF(false)} style={{width:"100%",background:C.surface,border:`1px solid ${f?C.blue:C.border}`,borderRadius:8,padding:"11px 13px",color:C.text,fontSize:11,lineHeight:1.7,resize:"vertical",fontFamily:"inherit",transition:"border-color 0.2s"}}/>{voice&&<div style={{position:"absolute",bottom:7,right:7}}><MicBtn onResult={t=>onChange({target:{value:value+(value?"\n":"")+t}})} sm/></div>}</div>{hint&&<div style={{fontSize:11,color:C.muted,marginTop:3}}>{hint}</div>}</div>);
}

function FSelect({label,value,onChange,options}){
  return(<div style={{marginBottom:0}}>{label&&<label style={{fontSize:11,letterSpacing:"0.08em",color:C.muted,display:"block",marginBottom:5,textTransform:"uppercase"}}>{label}</label>}<div style={{position:"relative"}}><select value={value} onChange={e=>onChange(e.target.value)} style={{width:"100%",background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,padding:"10px 32px 10px 12px",color:C.text,fontSize:11,fontFamily:"inherit",cursor:"pointer"}}>{options.map(o=><option key={o.value??o} value={o.value??o}>{o.label??o}</option>)}</select><span style={{position:"absolute",right:10,top:"50%",transform:"translateY(-50%)",pointerEvents:"none",color:C.muted,fontSize:11}}>▾</span></div></div>);
}
...
const PriBtn=({children,onClick,loading,disabled,fullWidth=true,variant="blue"})=>{
  ...
  return <button onClick={onClick} disabled={loading||disabled} style={{width:fullWidth?"100%":"auto",padding:"12px 20px",borderRadius:8,border:"none",background:bg,color:loading||disabled?C.muted:"#000",fontSize:11,fontWeight:800,cursor:loading||disabled?"not-allowed":"pointer",transition:"all 0.2s",display:"flex",alignItems:"center",justifyContent:"center",gap:8,boxShadow:loading||disabled?"none":variant==="violet"?"0 4px 20px rgba(155,127,232,0.3)":`0 4px 20px ${C.blueGlow}`,fontFamily:"inherit",letterSpacing:"0.01em"}}>{loading?<><Spin/> Processing...</>:children}</button>;
};

const SecBtn=({children,onClick})=>(<button onClick={onClick} style={{width:"100%",padding:"11px",borderRadius:8,background:"transparent",border:`1px solid ${C.border}`,color:C.muted,fontSize:11,fontWeight:600,cursor:"pointer",transition:"all 0.2s",fontFamily:"inherit"}} onMouseEnter={e=>{e.currentTarget.style.borderColor=C.blue;e.currentTarget.style.color=C.text;}} onMouseLeave={e=>{e.currentTarget.style.borderColor=C.border;e.currentTarget.style.color=C.muted;}}>{children}</button>);
...
function TermsModal({onClose}){
  return(<div style={{position:"fixed",inset:0,zIndex:500,background:C.bg,display:"flex",flexDirection:"column",animation:"fadeUp 0.2s ease",fontFamily:"'Cabinet Grotesk',sans-serif"}}><div style={{background:"rgba(0,0,0,0.98)",borderBottom:`1px solid ${C.border}`,padding:"13px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}><div style={{fontSize:11,fontWeight:800,color:"#fff"}}>Terms & Conditions</div><button onClick={onClose} style={{width:30,height:30,borderRadius:"50%",background:C.surface,border:`1px solid ${C.border}`,color:C.muted,fontSize:11,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>✕</button></div><div style={{flex:1,overflowY:"auto",padding:"20px 16px 48px",maxWidth:620,width:"100%",margin:"0 auto"}}>{TERMS_CONTENT.map((s,i)=>(<div key={i} style={{marginBottom:20}}><div style={{fontSize:11,fontWeight:700,color:C.blue,marginBottom:5}}>{s.h}</div><div style={{fontSize:11,color:C.muted,lineHeight:1.75}}>{s.b}</div>{i<TERMS_CONTENT.length-1&&<div style={{height:1,background:C.border,marginTop:16}}/>}</div>))}</div><div style={{padding:"13px 16px",borderTop:`1px solid ${C.border}`,background:"rgba(0,0,0,0.98)"}}><button onClick={onClose} style={{width:"100%",maxWidth:460,margin:"0 auto",display:"block",padding:"12px",borderRadius:8,background:`linear-gradient(135deg,${C.blue},${C.accent})`,color:"#000",fontSize:11,fontWeight:800,cursor:"pointer",border:"none",fontFamily:"inherit"}}>Got it — Close ✓</button></div></div>);
}

function SafetyScreen({onAccept}){
  const [c1,setC1]=useState(false);const [c2,setC2]=useState(false);const [c3,setC3]=useState(false);
  const all=c1&&c2&&c3;
  const CheckRow=({checked,set,children})=>(<div onClick={set} style={{display:"flex",alignItems:"flex-start",gap:10,padding:"12px",background:checked?C.accentSoft:C.surface,border:`1px solid ${checked?C.blue:C.border}`,borderRadius:9,cursor:"pointer",transition:"all 0.15s",marginBottom:8}}><div style={{width:17,height:17,borderRadius:4,border:`2px solid ${checked?C.blue:C.border}`,background:checked?C.blue:"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,marginTop:1,transition:"all 0.15s"}}>{checked&&<span style={{color:"#000",fontSize:11,fontWeight:900}}>✓</span>}</div><div style={{fontSize:11,color:checked?C.text:C.muted,lineHeight:1.6,transition:"color 0.15s"}}>{children}</div></div>);
  return(<div style={{minHeight:"100vh",background:C.bg,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"24px 16px",fontFamily:"'Cabinet Grotesk',sans-serif"}}><div style={{width:"100%",maxWidth:420,animation:"fadeUp 0.4s ease"}}><div style={{textAlign:"center",marginBottom:24}}><div style={{fontSize:48,marginBottom:10,animation:"glow 3s ease infinite"}}>🛡️</div><div style={{fontSize:11,fontWeight:900,color:"#fff",letterSpacing:"-0.01em",marginBottom:6}}>Before You Begin</div><div style={{fontSize:11,color:C.muted,lineHeight:1.6}}>Read and accept all three conditions to continue.</div></div><div style={{background:"rgba(245,200,66,0.06)",border:"1px solid rgba(245,200,66,0.2)",borderRadius:9,padding:"11px 13px",marginBottom:18,display:"flex",gap:8}}><span style={{fontSize:11,flexShrink:0}}>⚠️</span><div style={{fontSize:11,color:C.yellow,lineHeight:1.6}}>AI content is generated under <strong>your direction</strong>. You are solely responsible for how it is used.</div></div><CheckRow checked={c1} set={()=>setC1(!c1)}><strong style={{color:c1?"#fff":C.muted}}>I take full responsibility</strong> for all content I generate. GhostwriterMe is not liable.</CheckRow><CheckRow checked={c2} set={()=>setC2(!c2)}><strong style={{color:c2?"#fff":C.muted}}>I will not use this tool</strong> to create harmful, illegal, or deceptive content.</CheckRow><CheckRow checked={c3} set={()=>setC3(!c3)}><strong style={{color:c3?"#fff":C.muted}}>I understand AI output may contain errors</strong> and I will verify content before use.</CheckRow><div style={{display:"flex",gap:4,marginBottom:16,marginTop:4}}>{[c1,c2,c3].map((c,i)=><div key={i} style={{height:2,flex:1,borderRadius:1,background:c?C.blue:C.border,transition:"background 0.3s"}}/>)}</div><button onClick={onAccept} disabled={!all} style={{width:"100%",padding:"13px",borderRadius:8,border:"none",background:all?`linear-gradient(135deg,${C.blue},${C.accent})`:"#0c1220",color:all?"#000":C.muted,fontSize:11,fontWeight:800,cursor:all?"pointer":"not-allowed",transition:"all 0.3s",fontFamily:"inherit",boxShadow:all?`0 4px 20px ${C.blueGlow}`:"none"}}>{all?"I Agree — Enter GhostwriterMe →":"Accept "+[c1,c2,c3].filter(x=>!x).length+" more to continue"}</button></div></div>);
}
    ...
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"24px 16px",background:C.bg,fontFamily:"'Cabinet Grotesk',sans-serif"}}>
      ...
      <div style={{width:"100%",maxWidth:370,background:C.card,border:`1px solid ${C.border}`,borderRadius:14,padding:"22px 18px",animation:"fadeUp 0.4s 0.08s ease both",boxShadow:"0 20px 50px rgba(0,0,0,0.7)"}}>
        <div style={{display:"flex",background:C.surface,borderRadius:7,padding:3,marginBottom:20}}>
          {["signin","signup"].map(t=>(<button key={t} onClick={()=>{setTab(t);setShowEmail(false);setErrs({});setAgreed(false);setAge("");}} style={{flex:1,padding:"7px",borderRadius:5,border:"none",background:tab===t?C.blue:"transparent",color:tab===t?"#000":C.muted,fontSize:11,fontWeight:800,cursor:"pointer",transition:"all 0.2s",fontFamily:"inherit"}}>{t==="signin"?"Sign In":"Create Account"}</button>))}
        </div>
        {!showEmail?(
          <><div style={{fontSize:11,fontWeight:800,color:"#fff",marginBottom:3}}>{tab==="signin"?"Welcome back 👋":"Join GhostwriterMe"}</div><div style={{fontSize:11,color:C.muted,marginBottom:18,lineHeight:1.5}}>{tab==="signin"?"Access your AI writing suite.":"Free forever. No card needed."}</div>
          <div style={{display:"flex",flexDirection:"column",gap:8}}>{SOCIAL_PROVIDERS.map(s=>(<button key={s.id} onClick={()=>handleSocial(s.id)} disabled={!!loading} style={{display:"flex",alignItems:"center",justifyContent:"center",gap:10,padding:"15px",borderRadius:8,border:s.border||"none",background:s.bg,color:s.color,fontSize:11,fontWeight:700,cursor:loading?"not-allowed":"pointer",transition:"opacity 0.2s,transform 0.15s",opacity:loading&&loading!==s.id?0.4:1,fontFamily:"inherit"}} onMouseEnter={e=>{if(!loading)e.currentTarget.style.transform="scale(1.015)";}} onMouseLeave={e=>{e.currentTarget.style.transform="scale(1)";}}>
          ...
          <div style={{animation:"slideIn 0.25s ease"}}>
            ...
            {tab==="signup"&&(<div style={{marginBottom:12}}><div onClick={()=>{setAgreed(!agreed);if(errs.terms)setErrs({...errs,terms:""});}} style={{display:"flex",alignItems:"flex-start",gap:9,padding:"10px 12px",background:agreed?C.accentSoft:C.surface,border:`1px solid ${errs.terms?C.red:agreed?C.blue:C.border}`,borderRadius:8,cursor:"pointer",transition:"all 0.15s"}}><div style={{width:16,height:16,borderRadius:3,border:`2px solid ${agreed?C.blue:errs.terms?C.red:C.border}`,background:agreed?C.blue:"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,marginTop:1}}>{agreed&&<span style={{color:"#000",fontSize:11,fontWeight:900}}>✓</span>}</div><div style={{fontSize:11,color:C.muted,lineHeight:1.5}}>I agree to the{" "}<span onClick={e=>{e.stopPropagation();setShowTC(true);}} style={{color:C.blue,fontWeight:700,cursor:"pointer",textDecoration:"underline"}}>Terms & Conditions</span>{" "}and{" "}<span style={{color:C.blue,fontWeight:700,cursor:"pointer"}}>Privacy Policy</span></div></div>{errs.terms&&<div style={{fontSize:11,color:C.red,marginTop:3}}>{errs.terms}</div>}</div>)}
            <PriBtn loading={loading==="email"} onClick={handleSubmit}>{tab==="signin"?"Sign In →":"Create Account →"}</PriBtn>
          </div>
        )}
        <div style={{textAlign:"center",fontSize:11,color:C.muted,marginTop:14,lineHeight:1.6}}>By continuing you agree to our{" "}<span onClick={()=>setShowTC(true)} style={{color:C.blue,cursor:"pointer"}}>Terms</span>{" "}&amp;{" "}<span style={{color:C.blue,cursor:"pointer"}}>Privacy</span></div>
      </div>
    </div></>
  );
}

// CHANGES 2+3: Intro pricing. PRO_F no longer has academic. STU_F adds academic.
    ...
    <div style={{minHeight:"100vh",background:C.bg,padding:"24px 14px 80px",display:"flex",flexDirection:"column",alignItems:"center",fontFamily:"'Cabinet Grotesk',sans-serif"}}>
      <div style={{width:"100%",maxWidth:440}}>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:20,animation:"fadeUp 0.4s ease"}}>
          <div style={{width:38,height:38,borderRadius:"50%",background:`linear-gradient(135deg,${C.blue},${C.accent})`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:11}}>{user.avatar}</div>
          <div><div style={{fontSize:11,fontWeight:800,color:"#fff"}}>Hey {user.name.split(" ")[0]} 👋</div><div style={{fontSize:11,color:C.muted}}>{user.email}</div></div>
        </div>
        <div style={{fontSize:11,fontWeight:900,color:"#fff",letterSpacing:"-0.02em",marginBottom:4,animation:"fadeUp 0.4s 0.05s ease both"}}>Choose your plan</div>
        <div style={{fontSize:11,color:C.muted,marginBottom:18,animation:"fadeUp 0.4s 0.08s ease both"}}>All plans include voice input and text-to-speech.</div>
        <div style={{display:"flex",background:C.card,border:`1px solid ${C.border}`,borderRadius:9,padding:3,marginBottom:14,animation:"fadeUp 0.4s 0.1s ease both"}}>
          {tabs.map(t=>(<button key={t.id} onClick={()=>setTab(t.id)} style={{flex:1,padding:"9px 4px",borderRadius:7,border:"none",background:tab===t.id?t.color:"transparent",color:tab===t.id?"#000":C.muted,fontSize:11,fontWeight:800,cursor:"pointer",transition:"all 0.2s",fontFamily:"inherit"}}>{t.label}</button>))}
        </div>
        {tab==="pro"&&(<div style={{display:"flex",background:C.surface,border:`1px solid ${C.border}`,borderRadius:7,padding:3,marginBottom:12,animation:"fadeUp 0.2s ease"}}>{[{id:"monthly",label:"Monthly"},{id:"yearly",label:"Yearly"}].map(b=>(<button key={b.id} onClick={()=>setProBill(b.id)} style={{flex:1,padding:"7px",borderRadius:5,border:"none",background:proBill===b.id?C.blue:"transparent",color:proBill===b.id?"#000":C.muted,fontSize:11,fontWeight:700,cursor:"pointer",transition:"all 0.2s",fontFamily:"inherit"}}>{b.label}</button>))}</div>)}
        ...
        <div style={{background:tab==="student"?`linear-gradient(150deg,rgba(155,127,232,0.07),${C.card})`:tab==="pro"?`linear-gradient(150deg,rgba(121,186,236,0.08),${C.card})`:C.card,border:`1px solid ${tab==="student"?"rgba(155,127,232,0.4)":tab==="pro"?C.blue:C.border}`,borderRadius:12,padding:"18px",position:"relative",overflow:"hidden",boxShadow:tab==="student"?`0 0 28px ${C.violetGlow}`:tab==="pro"?`0 0 28px ${C.blueGlow}`:"none",marginBottom:14,animation:"fadeUp 0.3s ease"}}>
          ...
          <div style={{fontSize:11,letterSpacing:"0.12em",color:tabColor,textTransform:"uppercase",marginBottom:5}}>{tab.toUpperCase()}</div>
          <div style={{display:"flex",alignItems:"baseline",gap:4,marginBottom:2}}><span style={{fontSize:34,fontWeight:900,color:"#fff",lineHeight:1,letterSpacing:"-0.02em"}}>{price.main}</span><span style={{fontSize:11,color:C.muted}}>{price.per}</span></div>
          {price.sub&&<div style={{fontSize:11,color:C.green,marginBottom:price.intro?2:14}}>{price.sub}</div>}
          {price.intro&&<div style={{fontSize:11,color:C.muted,marginBottom:14}}>{price.intro}</div>}
          ...
          <ul style={{listStyle:"none",marginBottom:16,display:"flex",flexDirection:"column",gap:5,maxHeight:270,overflowY:"auto"}}>
            {features.map((feat,i)=>{const isProEx=tab==="pro"&&i>=freeCount;const isStuEx=tab==="student"&&i>=proCount;const isProBas=tab==="student"&&i>=freeCount&&i<proCount;return(<li key={feat} style={{fontSize:11,color:isStuEx?C.accent:isProEx||isProBas?C.text:C.muted,display:"flex",alignItems:"flex-start",gap:6}}><span style={{color:isStuEx?C.violet:isProEx?C.blue:isProBas?C.blue:C.green,flexShrink:0,marginTop:1}}>✓</span>{feat}</li>);})}
          </ul>
          {tab!=="free"&&(<div style={{background:tab==="student"?C.violetSoft:C.accentSoft,border:`1px solid ${tab==="student"?"rgba(155,127,232,0.2)":"rgba(121,186,236,0.2)"}`,borderRadius:8,padding:"9px 11px",marginBottom:13,display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:11}}>🎁</span><div><div style={{fontSize:11,fontWeight:700,color:"#fff"}}>3-day free trial</div><div style={{fontSize:11,color:C.muted}}>No charge until day 4 · Cancel anytime</div></div></div>)}
        ...
        <div style={{display:"flex",justifyContent:"center",gap:5,flexWrap:"wrap",animation:"fadeUp 0.4s 0.18s ease both"}}>
          {tabs.filter(t=>t.id!==tab).map(t=>(<button key={t.id} onClick={()=>setTab(t.id)} style={{padding:"4px 12px",borderRadius:20,border:`1px solid ${C.border}`,background:"transparent",color:C.muted,fontSize:11,cursor:"pointer",fontFamily:"inherit",transition:"all 0.15s"}} onMouseEnter={e=>{e.currentTarget.style.borderColor=t.color;e.currentTarget.style.color=t.color;}} onMouseLeave={e=>{e.currentTarget.style.borderColor=C.border;e.currentTarget.style.color=C.muted;}}>View {t.id==="student"?"Student":t.id==="pro"?"Pro":"Free"} plan</button>))}
        </div>
        <div style={{marginTop:24,textAlign:"center"}}><button onClick={onContact} style={{background:"transparent",border:"none",color:C.muted,fontSize:11,cursor:"pointer",fontFamily:"inherit",textDecoration:"underline"}}>Questions? Contact us ✉️</button></div>
      </div>
    </div>
  );
}

// CHANGE 4: Apple Pay removed. Intro pricing shown in summary.
function PaymentScreen({user,billing,targetPlan,onComplete}){
  ...
  if(step==="success")return(<div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",padding:"24px",background:C.bg,fontFamily:"'Cabinet Grotesk',sans-serif"}}><div style={{textAlign:"center",maxWidth:320,animation:"fadeUp 0.5s ease"}}><div style={{fontSize:64,marginBottom:12,animation:"pulse 2s ease infinite"}}>🎉</div><div style={{fontSize:30,fontWeight:900,color:"#fff",letterSpacing:"-0.02em",marginBottom:6}}>You're in!</div><div style={{fontSize:11,color:C.muted,lineHeight:1.7,marginBottom:22}}>{isStudent?"Student plan activated!":"3-day free trial started."}<br/>All features unlocked. 🚀</div><PriBtn onClick={onComplete} variant={isStudent?"violet":"blue"}>Enter the App →</PriBtn></div></div>);

  return(
    <div style={{minHeight:"100vh",background:C.bg,padding:"24px 14px 80px",display:"flex",flexDirection:"column",alignItems:"center",fontFamily:"'Cabinet Grotesk',sans-serif"}}>
      <div style={{width:"100%",maxWidth:420}}>
        <div style={{marginBottom:18}}><div style={{fontSize:11,fontWeight:900,color:"#fff",letterSpacing:"-0.01em"}}>Payment</div><div style={{fontSize:11,color:C.muted}}>Secure checkout · SSL encrypted · Cancel anytime</div></div>
        <Card style={{marginBottom:16}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
            <div><div style={{fontSize:11,fontWeight:800,color:"#fff"}}>GhostwriterMe {isStudent?"Student":"Pro"}</div><div style={{fontSize:11,color:C.muted,marginTop:1}}>{billing} · after 3-day trial</div>{introNote&&<div style={{fontSize:11,color:C.green,marginTop:4}}>{introNote}</div>}</div>
            <div style={{textAlign:"right"}}><div style={{fontSize:11,fontWeight:900,color:planColor}}>{priceDisplay.split(" ")[0]}</div><div style={{fontSize:11,color:C.green,marginTop:1}}>Today: $0.00 ✓</div></div>
          </div>
        </Card>
        <div style={{marginBottom:16}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {METHODS.map(m=>(<button key={m.id} onClick={()=>setMethod(m.id)} style={{display:"flex",alignItems:"center",gap:11,padding:"12px",background:method===m.id?C.accentSoft:C.card,border:`1px solid ${method===m.id?planColor:C.border}`,borderRadius:9,cursor:"pointer",transition:"all 0.2s",textAlign:"left",fontFamily:"inherit"}}><div style={{width:38,height:38,borderRadius:8,flexShrink:0,background:m.color,display:"flex",alignItems:"center",justifyContent:"center",fontSize:11}}>{m.icon}</div><div style={{flex:1}}><div style={{fontSize:11,fontWeight:700,color:"#fff"}}>{m.label}</div><div style={{fontSize:11,color:C.muted,marginTop:1}}>{m.desc}</div></div>{method===m.id&&<div style={{width:18,height:18,borderRadius:"50%",background:planColor,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><span style={{fontSize:11,color:"#000",fontWeight:900}}>✓</span></div>}</button>))}
          </div>
        </div>
        {method==="card"&&(<Card style={{marginBottom:14,animation:"slideIn 0.3s ease"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:11}}><div style={{fontSize:11,color:C.muted}}>Card details</div><div style={{display:"flex",gap:4}}>{["VISA","MC"].map(b=><div key={b} style={{background:brand(card.number)===b?"#fff":"transparent",border:`1px solid ${brand(card.number)===b?"#fff":C.border}`,borderRadius:3,padding:"1px 6px",fontSize:11,fontWeight:900,color:brand(card.number)===b?"#000":C.muted}}>{b}</div>)}</div></div><FInput label="Card Number" placeholder="1234 5678 9012 3456" icoL="💳" value={card.number} onChange={e=>setCard({...card,number:fmt4(e.target.value)})} error={cErr.number}/><FInput label="Cardholder Name" placeholder="As shown on card" icoL="👤" value={card.name} onChange={e=>setCard({...card,name:e.target.value})} error={cErr.name}/><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><FInput label="Expiry" placeholder="MM/YY" value={card.expiry} onChange={e=>setCard({...card,expiry:fmtExp(e.target.value)})} error={cErr.expiry}/><FInput label="CVV" type="password" placeholder="•••" value={card.cvv} onChange={e=>setCard({...card,cvv:e.target.value.replace(/\D/g,"").slice(0,4)})} error={cErr.cvv}/></div><div style={{fontSize:11,color:C.muted}}>🔒 Processed by Stripe. We never store card data.</div></Card>)}
        {method==="paypal"&&<Card style={{marginBottom:14,textAlign:"center",animation:"slideIn 0.3s ease"}}><div style={{fontSize:30,marginBottom:7}}>🅿️</div><div style={{fontSize:11,fontWeight:700,color:"#fff",marginBottom:3}}>PayPal</div><div style={{fontSize:11,color:C.muted}}>You'll be redirected to PayPal to approve.</div></Card>}
        {method&&(<div style={{animation:"fadeUp 0.3s ease"}}><PriBtn loading={loading} onClick={handlePay} variant={isStudent?"violet":"blue"}>{method==="paypal"?"Continue to PayPal →":"Confirm & Start Free Trial →"}</PriBtn><div style={{textAlign:"center",fontSize:11,color:C.muted,marginTop:7}}>🔒 Secure · No charge today · Cancel anytime</div></div>)}
      </div>
    </div>
  );
}
...
function HistoryMode({user}){
  ...
  if(!items.length)return(<div style={{textAlign:"center",padding:"44px 0"}}><div style={{fontSize:40,marginBottom:10}}>🕐</div><div style={{fontSize:11,fontWeight:700,color:"#fff",marginBottom:5}}>No history yet</div><div style={{fontSize:11,color:C.muted}}>Generated content will appear here.</div></div>);
  return(<div><div style={{display:"flex",gap:5,marginBottom:14,overflowX:"auto",paddingBottom:3}}>{["all",...Object.keys(ML)].map(m=>(<button key={m} onClick={()=>setFilter(m)} style={{flexShrink:0,padding:"5px 10px",borderRadius:20,border:`1px solid ${filter===m?C.blue:C.border}`,background:filter===m?C.accentSoft:"transparent",color:filter===m?C.blue:C.muted,fontSize:11,fontWeight:600,cursor:"pointer",fontFamily:"inherit",transition:"all 0.15s"}}>{m==="all"?"All":(MI[m]||"")+" "+(ML[m]||m)}</button>))}</div><div style={{fontSize:11,color:C.muted,marginBottom:9}}>{filtered.length} item{filtered.length!==1?"s":""}</div>{filtered.map(item=>(<Card key={item.id} style={{marginBottom:8}}><div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:exp===item.id?9:0}}><div style={{display:"flex",alignItems:"center",gap:7,flex:1,minWidth:0}}><span style={{fontSize:11,flexShrink:0}}>{MI[item.mode]||"📝"}</span><div style={{minWidth:0}}><div style={{fontSize:11,color:C.muted}}>{ML[item.mode]||item.mode} · {new Date(item.ts).toLocaleDateString("en-GB",{day:"2-digit",month:"short",hour:"2-digit",minute:"2-digit"})}</div><div style={{fontSize:11,color:C.text,fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{item.title||item.output?.slice(0,55)||"Untitled"}</div></div></div><button onClick={()=>setExp(exp===item.id?null:item.id)} style={{flexShrink:0,padding:"4px 8px",borderRadius:5,border:`1px solid ${C.border}`,background:"transparent",color:C.muted,fontSize:11,cursor:"pointer",fontFamily:"inherit",marginLeft:6}}>{exp===item.id?"Hide":"View"}</button></div>{exp===item.id&&(<div style={{animation:"fadeUp 0.2s ease"}}>{item.input&&<div style={{fontSize:11,color:C.muted,background:C.surface,borderRadius:6,padding:"7px 10px",marginBottom:7,lineHeight:1.5}}><strong>Input:</strong> {item.input}</div>}<div style={{fontSize:11,lineHeight:1.8,color:C.text,whiteSpace:"pre-wrap",maxHeight:200,overflowY:"auto",background:C.surface,borderRadius:6,padding:"9px 11px"}}>{item.output}</div><OutputActions text={item.output}/></div>)}</Card>))}</div>);
}
...
function ReplyMode({user,isPro}){
  ...
  return(
    <div>
      {!isPro&&<div style={{background:C.accentSoft,border:`1px solid ${C.border}`,borderRadius:7,padding:"8px 12px",marginBottom:14,display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{fontSize:11,color:C.muted}}>{FREE_LIMIT-used} free replies left today</span><span style={{fontSize:11,color:C.blue,fontWeight:700}}>Upgrade →</span></div>}
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}}>
        {TONES.map(t=>(<button key={t.id} onClick={()=>setTone(t.id)} style={{background:tone===t.id?C.accentSoft:C.surface,border:`1px solid ${tone===t.id?C.blue:C.border}`,borderRadius:8,padding:"9px 10px",cursor:"pointer",textAlign:"left",color:C.text,fontFamily:"inherit",transition:"all 0.15s"}}><div style={{fontSize:11}}>{t.emoji}</div><div style={{fontSize:11,fontWeight:700,marginTop:3}}>{t.label}</div><div style={{fontSize:11,color:C.muted,marginTop:1}}>{t.desc}</div></button>))}
      </div>
      <div onClick={()=>setNoDesp(!noDesp)} style={{background:noDesp?C.accentSoft:C.surface,border:`1px solid ${noDesp?C.blue:C.border}`,borderRadius:8,padding:"11px 13px",display:"flex",alignItems:"center",justifyContent:"space-between",cursor:"pointer",transition:"all 0.15s",marginBottom:13}}>
        <div><div style={{fontSize:11,fontWeight:700}}>💀 Don't Sound Desperate</div><div style={{fontSize:11,color:C.muted,marginTop:2}}>Strips clingy energy. Unbothered only.</div></div>
      ...
      {replies.length>0&&(<div ref={ref} style={{marginTop:20,animation:"fadeUp 0.4s ease"}}>{replies.map((r,i)=>(<Card key={i} style={{marginTop:9}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}><span style={{fontSize:11,color:C.accent,textTransform:"uppercase",letterSpacing:"0.1em"}}>{r.vibe}</span><span style={{fontSize:11,color:C.muted}}>Option {r.option}</span></div><div style={{fontSize:11,lineHeight:1.7,color:C.text}}>{r.text}</div><div style={{display:"flex",gap:7,marginTop:11,flexWrap:"wrap"}}><CopyBtn text={r.text}/><ListenBtn text={r.text}/><SaveAsImageBtn text={r.text} title="AI Reply"/></div></Card>))}</div>)}
    </div>
  );
}
...
function GrammarMode({user}){
  ...
  return(<div><FArea label="Paste Your Text" placeholder="Any text — email, essay, message..." value={text} onChange={e=>setText(e.target.value)} rows={6} voice/><div style={{marginBottom:12}}><div style={{fontSize:11,letterSpacing:"0.1em",color:C.muted,textTransform:"uppercase",marginBottom:8}}>Rewrite Style</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>{GRAMMAR_STYLES.map(s=><button key={s.id} onClick={()=>setStyle(s.id)} style={{background:style===s.id?C.accentSoft:C.surface,border:`1px solid ${style===s.id?C.blue:C.border}`,borderRadius:8,padding:"11px 7px",cursor:"pointer",textAlign:"center",color:C.text,fontFamily:"inherit",transition:"all 0.15s"}}><div style={{fontSize:11,marginBottom:4}}>{s.icon}</div><div style={{fontSize:11,fontWeight:700}}>{s.label}</div><div style={{fontSize:11,color:C.muted,marginTop:2,lineHeight:1.3}}>{s.desc}</div></button>)}</div></div><ImageInput onImage={(d,t)=>{setImgData(d);setImgType(t);}} imageData={imgData} onClear={()=>{setImgData(null);setImgType(null);}}/><PriBtn onClick={check} loading={loading} disabled={!text.trim()}>✅ Check & Rewrite</PriBtn>{error&&<ErrBox msg={error}/>}{res&&<div style={{marginTop:16,animation:"fadeUp 0.4s ease"}}><Card style={{marginBottom:9,display:"flex",alignItems:"center",gap:14}}><div style={{width:54,height:54,borderRadius:"50%",border:`3px solid ${sc}`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",flexShrink:0}}><span style={{fontSize:11,fontWeight:900,color:sc,lineHeight:1}}>{res.score}</span><span style={{fontSize:11,color:C.muted}}>SCORE</span></div><div><div style={{fontSize:11,color:C.text,marginBottom:2}}>{res.summary}</div><div style={{fontSize:11,color:C.muted}}>{res.errors?.length||0} issue{res.errors?.length!==1?"s":""} found</div></div></Card>{res.errors?.length>0&&<Card style={{marginBottom:9}}><div style={{fontSize:11,color:C.red,textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:8}}>Issues Found</div>{res.errors.map((e,i)=>{const tc={grammar:C.red,spelling:"#93c5fd",punctuation:C.green,style:"#c4b5fd"}[e.type]||C.muted;return<div key={i} style={{padding:"8px 0",borderBottom:i<res.errors.length-1?`1px solid ${C.border}`:"none"}}><span style={{fontSize:11,letterSpacing:"0.1em",textTransform:"uppercase",color:tc,background:tc+"22",padding:"2px 5px",borderRadius:3}}>{e.type}</span><div style={{display:"flex",gap:6,fontSize:11,marginTop:5,marginBottom:2,flexWrap:"wrap",alignItems:"center"}}><span style={{color:C.red,textDecoration:"line-through"}}>{e.original}</span><span style={{color:C.muted}}>→</span><span style={{color:C.green}}>{e.fixed}</span></div><div style={{fontSize:11,color:C.muted}}>{e.explanation}</div></div>;})}</Card>}<Card><div style={{fontSize:11,color:C.accent,textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:8}}>Rewritten — {GRAMMAR_STYLES.find(s=>s.id===style)?.label}</div><div style={{fontSize:11,lineHeight:1.85,color:C.text,whiteSpace:"pre-wrap"}}>{res.rewritten}</div><div style={{display:"flex",gap:7,marginTop:11,flexWrap:"wrap"}}><CopyBtn text={res.rewritten}/><ListenBtn text={res.rewritten}/><SaveAsImageBtn text={res.rewritten} title="Grammar Rewrite"/></div></Card></div>}</div>);
}
...
function EssayMode({user}){
  ...
  return(<div><FArea label="Essay Topic" placeholder="e.g. The impact of social media on mental health" value={topic} onChange={e=>setTopic(e.target.value)} rows={2} voice/><FArea label="Key Points (optional)" placeholder="e.g. Stats, comparisons, case studies..." value={details} onChange={e=>setDetails(e.target.value)} rows={3} voice/><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}}><FSelect label="Essay Type" value={type} onChange={setType} options={ESSAY_TYPES}/><FSelect label="Word Count" value={wc} onChange={setWc} options={["300","500","750","1000","1500","2000"].map(n=>({value:n,label:n+" words"}))}/></div><div style={{marginBottom:13}}><div style={{fontSize:11,letterSpacing:"0.1em",color:C.muted,textTransform:"uppercase",marginBottom:7}}>English Level (CEFR)</div><div style={{display:"flex",gap:5}}>{LEVELS.map(l=><button key={l} onClick={()=>setLevel(l)} style={{flex:1,padding:"7px 2px",borderRadius:6,background:level===l?C.accentSoft:C.surface,border:`1px solid ${level===l?C.blue:C.border}`,color:level===l?"#fff":C.muted,fontSize:11,fontWeight:level===l?800:400,cursor:"pointer",fontFamily:"inherit",transition:"all 0.15s"}}>{l}</button>)}</div><div style={{fontSize:11,color:C.muted,marginTop:4}}>{LD[level]}</div></div><ImageInput onImage={(d,t)=>{setImgData(d);setImgType(t);}} imageData={imgData} onClear={()=>{setImgData(null);setImgType(null);}}/><PriBtn onClick={gen} loading={loading} disabled={!topic.trim()}>✍️ Generate Essay</PriBtn>{error&&<ErrBox msg={error}/>}{essay&&<Card style={{marginTop:16,animation:"fadeUp 0.4s ease"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:11}}><span style={{fontSize:11,color:C.accent,textTransform:"uppercase",letterSpacing:"0.1em"}}>{type} · {level}</span><span style={{fontSize:11,color:C.muted}}>~{essay.split(/\s+/).length}w</span></div><div style={{fontSize:11,lineHeight:1.9,color:C.text,whiteSpace:"pre-wrap"}}>{essay}</div><div style={{display:"flex",gap:7,marginTop:11,flexWrap:"wrap"}}><CopyBtn text={essay}/><ListenBtn text={essay}/><SaveAsImageBtn text={essay} title={type+" Essay"}/></div></Card>}</div>);
}
...
function AcademicMode({user}){
  ...
  return(<div><FArea label="Thesis / Topic" placeholder="e.g. The role of AI in modern healthcare" value={topic} onChange={e=>setTopic(e.target.value)} rows={2} voice/><FArea label="Arguments & Key Points" placeholder="e.g. ML accuracy, ethical concerns..." value={details} onChange={e=>setDetails(e.target.value)} rows={3} voice/><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:14}}><FSelect label="Citation Style" value={style} onChange={setStyle} options={["APA","MLA","Chicago","Harvard","Vancouver","IEEE"]}/><FSelect label="Word Count" value={wc} onChange={setWc} options={["500","750","1000","1500","2000","3000"].map(n=>({value:n,label:n+" words"}))}/></div><div style={{marginBottom:12}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}><div style={{fontSize:11,letterSpacing:"0.1em",color:C.muted,textTransform:"uppercase"}}>Sources to Cite</div><button onClick={addC} style={{background:"transparent",border:`1px solid ${C.border}`,borderRadius:5,padding:"3px 9px",color:C.blue,fontSize:11,cursor:"pointer",fontFamily:"inherit"}}>+ Add</button></div>{cites.map((c,i)=>(<div key={i} style={{display:"flex",gap:6,marginBottom:7,alignItems:"center"}}><select value={c.type} onChange={e=>updC(i,"type",e.target.value)} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:6,padding:"8px 6px",color:C.text,fontSize:11,fontFamily:"inherit",width:76,flexShrink:0}}><option value="url">🔗 URL</option><option value="pdf">📄 PDF</option></select><input value={c.value} onChange={e=>updC(i,"value",e.target.value)} placeholder={c.type==="url"?"https://...":"Author, Title, Year..."} style={{flex:1,background:C.surface,border:`1px solid ${C.border}`,borderRadius:6,padding:"8px 10px",color:C.text,fontSize:11,fontFamily:"inherit"}} onFocus={e=>e.target.style.borderColor=C.blue} onBlur={e=>e.target.style.borderColor=C.border}/>{cites.length>1&&<button onClick={()=>remC(i)} style={{background:"none",border:"none",color:C.muted,cursor:"pointer",fontSize:11,flexShrink:0}}>✕</button>}</div>))}</div><ImageInput onImage={(d,t)=>{setImgData(d);setImgType(t);}} imageData={imgData} onClear={()=>{setImgData(null);setImgType(null);}}/><PriBtn onClick={gen} loading={loading} disabled={!topic.trim()}>🎓 Generate Academic Essay</PriBtn>{error&&<ErrBox msg={error}/>}{essay&&<Card style={{marginTop:16,animation:"fadeUp 0.4s ease"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:11}}><span style={{fontSize:11,color:C.accent,textTransform:"uppercase",letterSpacing:"0.1em"}}>Academic · {style}</span><span style={{fontSize:11,color:C.muted}}>~{essay.split(/\s+/).length}w</span></div><div style={{fontSize:11,lineHeight:2,color:C.text,whiteSpace:"pre-wrap",fontFamily:"'Instrument Serif',Georgia,serif"}}>{essay}</div><div style={{display:"flex",gap:7,marginTop:11,flexWrap:"wrap"}}><CopyBtn text={essay}/><ListenBtn text={essay}/><SaveAsImageBtn text={essay} title={"Academic Essay · "+style}/></div></Card>}</div>);
}
...
function CVMode({user}){
  ...
  return(<div><div style={{fontSize:11,letterSpacing:"0.1em",color:C.muted,textTransform:"uppercase",marginBottom:8}}>CV Style</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:7,marginBottom:12}}>{CVS.map(s=><button key={s.id} onClick={()=>setCvs(s.id)} style={{background:cvs===s.id?C.accentSoft:C.surface,border:`1px solid ${cvs===s.id?C.blue:C.border}`,borderRadius:8,padding:"9px 10px",cursor:"pointer",textAlign:"left",color:C.text,fontFamily:"inherit",transition:"all 0.15s"}}><div style={{fontSize:11}}>{s.icon}</div><div style={{fontSize:11,fontWeight:700,marginTop:2}}>{s.label}</div><div style={{fontSize:11,color:C.muted,marginTop:1}}>{s.desc}</div></button>)}</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:3}}><FSelect label="Generate" value={sec} onChange={setSec} options={[{value:"full",label:"Full CV"},...CV_SECTIONS.map(s=>({value:s,label:s+" only"}))]}/><FInput label="Target Role" placeholder="e.g. Senior PM" value={tr} onChange={e=>setTr(e.target.value)} icoL="🎯"/></div><FInput label="Current Job Title" placeholder="e.g. Product Manager" icoL="💼" value={jt} onChange={e=>setJt(e.target.value)}/><FInput label="Industry" placeholder="e.g. FinTech, Healthcare" icoL="🏢" value={ind} onChange={e=>setInd(e.target.value)}/><FArea label="Work Experience" placeholder="Company, title, years, responsibilities..." value={exp} onChange={e=>setExp(e.target.value)} rows={4} voice/><FArea label="Key Skills" placeholder="e.g. Python, Figma, leadership..." value={ski} onChange={e=>setSki(e.target.value)} rows={2} voice/><FArea label="Education" placeholder="e.g. BSc CS, Chulalongkorn, 2019–2023" value={edu} onChange={e=>setEdu(e.target.value)} rows={2}/><FArea label="Achievements (optional)" placeholder="e.g. Grew revenue 40%..." value={ach} onChange={e=>setAch(e.target.value)} rows={2} voice/><ImageInput onImage={(d,t)=>{setImgData(d);setImgType(t);}} imageData={imgData} onClear={()=>{setImgData(null);setImgType(null);}}/><PriBtn onClick={gen} loading={loading} disabled={!jt.trim()&&!exp.trim()}>💼 Build My CV</PriBtn>{error&&<ErrBox msg={error}/>}{res&&<Card style={{marginTop:16,animation:"fadeUp 0.4s ease"}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:11}}><span style={{fontSize:11,color:C.accent,textTransform:"uppercase",letterSpacing:"0.1em"}}>{cvs} CV</span><span style={{fontSize:11,color:C.muted}}>~{res.split(/\s+/).length}w</span></div><div style={{fontSize:11,lineHeight:1.9,color:C.text,whiteSpace:"pre-wrap",fontFamily:"'Instrument Serif',Georgia,serif"}}>{res}</div><div style={{display:"flex",gap:7,marginTop:11,flexWrap:"wrap"}}><CopyBtn text={res}/><ListenBtn text={res}/><SaveAsImageBtn text={res} title={cvs+" CV"}/></div></Card>}</div>);
}
  ...
  return(
    <div>
      <div style={{background:C.violetSoft,border:"1px solid rgba(155,127,232,0.28)",borderRadius:8,padding:"11px 13px",marginBottom:14,display:"flex",gap:9}}>
        ...
        <div><div style={{fontSize:11,fontWeight:800,color:C.violet,marginBottom:2}}>Humanize My Writing — Student Exclusive</div><div style={{fontSize:11,color:C.muted,lineHeight:1.5}}>Two-pass AI removal. Strips em dashes, robotic transitions, buzzwords, and uniform sentence patterns.</div></div>
      </div>
      ...
      <div style={{marginBottom:13}}><div style={{fontSize:11,letterSpacing:"0.1em",color:C.muted,textTransform:"uppercase",marginBottom:7}}>Your English Level (CEFR)</div><div style={{display:"flex",gap:5}}>{LEVELS.map(l=>(<button key={l} onClick={()=>setLevel(l)} style={{flex:1,padding:"7px 2px",borderRadius:6,background:level===l?C.violetSoft:C.surface,border:`1px solid ${level===l?C.violet:C.border}`,color:level===l?C.violet:C.muted,fontSize:11,fontWeight:level===l?800:400,cursor:"pointer",fontFamily:"inherit",transition:"all 0.15s"}}>{l}</button>))}</div><div style={{fontSize:11,color:C.muted,marginTop:4}}>{LD[level]}</div></div>
      <div style={{marginBottom:13}}><div style={{fontSize:11,letterSpacing:"0.1em",color:C.muted,textTransform:"uppercase",marginBottom:8}}>Writing Purpose</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:7}}>{PURPOSES.map(p=>(<button key={p.id} onClick={()=>setPurpose(p.id)} style={{background:purpose===p.id?C.violetSoft:C.surface,border:`1px solid ${purpose===p.id?C.violet:C.border}`,borderRadius:8,padding:"9px 10px",cursor:"pointer",textAlign:"left",color:C.text,fontFamily:"inherit",transition:"all 0.15s"}}><div style={{fontSize:11}}>{p.icon}</div><div style={{fontSize:11,fontWeight:700,marginTop:2}}>{p.label}</div><div style={{fontSize:11,color:C.muted,marginTop:1}}>{p.desc}</div></button>))}</div></div>
      <div style={{marginBottom:14}}><div style={{fontSize:11,letterSpacing:"0.1em",color:C.muted,textTransform:"uppercase",marginBottom:8}}>Transformation Intensity</div>{INTENSITIES.map(iv=>(<div key={iv.id} onClick={()=>setIntensity(iv.id)} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 12px",background:intensity===iv.id?C.violetSoft:C.surface,border:`1px solid ${intensity===iv.id?C.violet:C.border}`,borderRadius:8,cursor:"pointer",transition:"all 0.15s",marginBottom:6}}><div><div style={{fontSize:11,fontWeight:700,color:intensity===iv.id?C.violet:C.text}}>{iv.label}</div><div style={{fontSize:11,color:C.muted,marginTop:1}}>{iv.desc}</div></div><div style={{width:16,height:16,borderRadius:"50%",border:`2px solid ${intensity===iv.id?C.violet:C.border}`,background:intensity===iv.id?C.violet:"transparent",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center"}}>{intensity===iv.id&&<div style={{width:7,height:7,borderRadius:"50%",background:"#000"}}/>}</div></div>))}</div>
      <button onClick={process} disabled={isLoading||!text.trim()} style={{width:"100%",padding:"13px",borderRadius:8,border:"none",background:isLoading||!text.trim()?"#0c1220":"linear-gradient(135deg,#9b7fe8,#c4b5fd)",color:isLoading||!text.trim()?C.muted:"#000",fontSize:11,fontWeight:800,cursor:isLoading||!text.trim()?"not-allowed":"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center",gap:9,transition:"all 0.2s",boxShadow:isLoading||!text.trim()?"none":"0 4px 20px rgba(155,127,232,0.3)"}}>
        {isLoading?(<><Spin color="#9b7fe8"/><span style={{color:C.violet}}>{loadingLabel}</span></>):"🧠 Humanize My Writing"}
      </button>
      {isLoading&&(<div style={{marginTop:10,display:"flex",gap:6,alignItems:"center"}}><div style={{flex:1,height:3,borderRadius:2,background:phase==="pass1"||phase==="pass2"?"rgba(155,127,232,0.6)":C.border,transition:"background 0.4s"}}/><div style={{flex:1,height:3,borderRadius:2,background:phase==="pass2"?"rgba(155,127,232,0.6)":C.border,transition:"background 0.4s"}}/><div style={{fontSize:11,color:C.violet,flexShrink:0}}>{phase==="pass1"?"1 of 2":"2 of 2"}</div></div>)}
      {error&&<ErrBox msg={error}/>}
      {res&&(<div style={{marginTop:16,animation:"fadeUp 0.4s ease"}}>
        {res.note&&<div style={{background:C.violetSoft,border:"1px solid rgba(155,127,232,0.2)",borderRadius:8,padding:"9px 12px",marginBottom:10,display:"flex",gap:8}}><span style={{fontSize:11}}>💡</span><div style={{fontSize:11,color:C.violet,lineHeight:1.6}}>{res.note}</div></div>}
        {res.changes?.length>0&&(<Card style={{marginBottom:10,borderColor:"rgba(155,127,232,0.3)"}}><div style={{fontSize:11,color:C.violet,textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:9}}>What Changed</div>{res.changes.map((c,i)=>(<div key={i} style={{display:"flex",gap:8,paddingBottom:i<res.changes.length-1?8:0,marginBottom:i<res.changes.length-1?8:0,borderBottom:i<res.changes.length-1?`1px solid ${C.border}`:"none"}}><span style={{color:C.violet,flexShrink:0,fontSize:11,marginTop:1}}>✓</span><div><div style={{fontSize:11,fontWeight:700,color:C.text}}>{c.what}</div><div style={{fontSize:11,color:C.muted,marginTop:1}}>{c.why}</div></div></div>))}</Card>)}
        <div style={{display:"flex",background:C.surface,borderRadius:7,padding:3,marginBottom:10}}>{[{id:"output",label:"✅ Final Output"},{id:"compare",label:"🔍 Before vs After"}].map(v=>(<button key={v.id} onClick={()=>setView(v.id)} style={{flex:1,padding:"7px",borderRadius:5,border:"none",background:view===v.id?C.violet:"transparent",color:view===v.id?"#000":C.muted,fontSize:11,fontWeight:700,cursor:"pointer",transition:"all 0.2s",fontFamily:"inherit"}}>{v.label}</button>))}</div>
        {view==="output"&&(<Card glow glowColor={C.violet}><div style={{fontSize:11,color:C.violet,textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:8}}>Humanized Output · 2-Pass Reviewed</div><div style={{fontSize:11,lineHeight:1.9,color:C.text,whiteSpace:"pre-wrap"}}>{res.humanized}</div><div style={{display:"flex",gap:7,marginTop:11,flexWrap:"wrap"}}><CopyBtn text={res.humanized}/><ListenBtn text={res.humanized}/><SaveAsImageBtn text={res.humanized} title="Humanized Writing"/></div></Card>)}
        {view==="compare"&&(<div><div style={{display:"flex",gap:10,marginBottom:8}}><div style={{display:"flex",alignItems:"center",gap:5,fontSize:11,color:C.muted}}><div style={{width:10,height:10,borderRadius:2,background:"rgba(240,107,107,0.25)",border:"1px solid rgba(240,107,107,0.5)"}}/>Original</div><div style={{display:"flex",alignItems:"center",gap:5,fontSize:11,color:C.muted}}><div style={{width:10,height:10,borderRadius:2,background:"rgba(155,127,232,0.25)",border:"1px solid rgba(155,127,232,0.5)"}}/>Changed words</div></div><div style={{marginBottom:10}}><div style={{fontSize:11,color:C.red,textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:6}}>Before</div><div style={{background:"rgba(240,107,107,0.05)",border:"1px solid rgba(240,107,107,0.2)",borderRadius:8,padding:"12px 14px",fontSize:11,lineHeight:1.9,color:C.text,whiteSpace:"pre-wrap"}}>{text}</div></div><div style={{marginBottom:10}}><div style={{fontSize:11,color:C.violet,textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:6}}>After</div><div style={{background:"rgba(155,127,232,0.05)",border:"1px solid rgba(155,127,232,0.25)",borderRadius:8,padding:"12px 14px",fontSize:11,lineHeight:1.9,color:C.text,whiteSpace:"pre-wrap"}}>{diffWords(text,res.humanized).map((w,i)=>(<span key={i}><span style={{background:w.changed?"rgba(155,127,232,0.22)":"transparent",borderRadius:w.changed?3:0,padding:w.changed?"1px 2px":0,color:w.changed?C.violet:C.text,fontWeight:w.changed?700:400}}>{w.word}</span>{i<res.humanized.split(/\s+/).length-1?" ":""}</span>))}</div></div><div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:10}}>{[{label:"Original words",val:text.split(/\s+/).filter(Boolean).length},{label:"Final words",val:res.humanized.split(/\s+/).filter(Boolean).length},{label:"Words changed",val:diffWords(text,res.humanized).filter(w=>w.changed).length},{label:"Change rate",val:Math.round(diffWords(text,res.humanized).filter(w=>w.changed).length/Math.max(res.humanized.split(/\s+/).filter(Boolean).length,1)*100)+"%"}].map(s=>(<div key={s.label} style={{flex:1,minWidth:70,background:C.surface,border:`1px solid ${C.border}`,borderRadius:7,padding:"8px 10px",textAlign:"center"}}><div style={{fontSize:11,fontWeight:900,color:C.violet}}>{s.val}</div><div style={{fontSize:11,color:C.muted,marginTop:2}}>{s.label}</div></div>))}</div><OutputActions text={res.humanized}/></div>)}
      </div>)}
    </div>
  );
}

// CHANGES 2+3: TrialModal with intro pricing ($7→$12, $15→$20)
    ...
    <div style={{position:"fixed",inset:0,zIndex:200,display:"flex",alignItems:"flex-end",justifyContent:"center",background:"rgba(0,0,0,0.8)",backdropFilter:"blur(6px)",animation:"fadeUp 0.2s ease"}} onClick={e=>{if(e.target===e.currentTarget)onClose();}}>
      <div style={{width:"100%",maxWidth:500,background:C.card,border:`1px solid ${isStudent?"rgba(155,127,232,0.4)":C.border}`,borderRadius:"14px 14px 0 0",padding:"20px 16px 28px",animation:"slideUpModal 0.3s ease",maxHeight:"92vh",overflowY:"auto",fontFamily:"'Cabinet Grotesk',sans-serif"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:11,marginBottom:15}}>
          <div style={{width:44,height:44,borderRadius:10,background:isStudent?"linear-gradient(135deg,#9b7fe8,#c4b5fd)":`linear-gradient(135deg,${C.blue},${C.accent})`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,flexShrink:0}}>{h.icon}</div>
          <div><div style={{fontSize:11,fontWeight:900,color:"#fff",letterSpacing:"-0.01em"}}>{h.title}</div><div style={{fontSize:11,color:C.muted,marginTop:1}}>{isStudent?"Student plan exclusive":"Unlock with a free trial"}</div></div>
        </div>
        {isStudent&&<div style={{background:C.violetSoft,border:"1px solid rgba(155,127,232,0.22)",borderRadius:7,padding:"9px 11px",marginBottom:12,fontSize:11,color:C.violet,lineHeight:1.5}}>🎓 Student exclusive — includes Academic Essay + Humanize My Writing tools.</div>}
        <div style={{background:C.surface,borderRadius:9,padding:"11px 13px",marginBottom:14}}>{h.perks.map(p=><div key={p} style={{display:"flex",gap:8,fontSize:11,color:C.text,padding:"3px 0"}}><span style={{color:isStudent?C.violet:C.green,flexShrink:0}}>✓</span>{p}</div>)}</div>
        ...
        <div style={{background:isStudent?"rgba(155,127,232,0.07)":C.accentSoft,border:`1px solid ${isStudent?"rgba(155,127,232,0.25)":"rgba(121,186,236,0.22)"}`,borderRadius:10,padding:"13px",marginBottom:12}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:11}}>
            <div>
              <div style={{fontSize:11,fontWeight:900,color:"#fff",letterSpacing:"-0.02em"}}>
                ...
                <span style={{fontSize:11,color:C.muted,fontWeight:400}}>{isStudent?" / 2 months":bill==="monthly"?" / month":" / year"}</span>
              </div>
              <div style={{fontSize:11,color:C.green,marginTop:1}}>{isStudent?"Intro offer · then $20 / 2 months":bill==="monthly"?"Intro offer · then $12 / month":"Best annual rate"}</div>
              <div style={{fontSize:11,color:C.muted,marginTop:1}}>billed in USD</div>
            </div>
            <div style={{background:isStudent?C.violetSoft:C.accentSoft,border:`1px solid ${planColor}44`,borderRadius:6,padding:"6px 9px",textAlign:"center"}}>
              <div style={{fontSize:11,color:planColor,fontWeight:800}}>🎁 3 DAYS FREE</div>
          ...
          <div style={{textAlign:"center",fontSize:11,color:C.muted,marginTop:7}}>Cancel anytime · No charge until day 4</div>
        </div>
...
// CHANGE 1 (tabs) + intro pricing navbar buttons
function AppShell({user,onStartTrial,onSignOut}){
  const [mode,setMode]=useState("reply");const [trialMode,setTMode]=useState(null);const [trialPlan,setTPlan]=useState("pro");const [showContact,setShowContact]=useState(false);
  const isPro=user.plan==="pro"||user.plan==="student";const isStudent=user.plan==="student";const planColor=isStudent?C.violet:C.blue;
  const canAccess=m=>{if(m.access==="free")return true;if(m.access==="pro+student")return isPro;if(m.access==="student")return isStudent;return false;};
  const handleTab=m=>{if(!canAccess(m)){setTPlan(m.access==="student"?"student":"pro");setTMode(m.id);}else setMode(m.id);};
  const handleStart=(bill,plan)=>{setTMode(null);onStartTrial(bill,plan);};
  const badgeFor=m=>{if(m.id==="history")return null;if(canAccess(m))return null;if(m.access==="student")return<span style={{background:C.violetSoft,color:C.violet,fontSize:11,fontWeight:800,padding:"2px 6px",borderRadius:3,textTransform:"uppercase",flexShrink:0}}>🎓</span>;return<span style={{background:C.accentSoft,color:C.blue,fontSize:11,fontWeight:800,padding:"1px 5px",borderRadius:3,textTransform:"uppercase",flexShrink:0}}>PRO</span>;};
  return(
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'Cabinet Grotesk',sans-serif",color:C.text,paddingBottom:80}}>
      ...
      <div style={{position:"sticky",top:0,zIndex:50,background:"rgba(0,0,0,0.95)",backdropFilter:"blur(14px)",borderBottom:`1px solid ${C.border}`,padding:"10px 13px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <div><div style={{fontSize:11,fontWeight:900,color:"#fff",letterSpacing:"-0.01em",lineHeight:1}}>👻 GhostwriterMe</div><div style={{fontSize:11,color:C.muted,letterSpacing:"0.15em"}}>AI WRITING SUITE</div></div>
        <div style={{display:"flex",alignItems:"center",gap:7}}>
          <button onClick={()=>setShowContact(true)} style={{padding:"4px 10px",borderRadius:20,border:`1px solid ${C.border}`,background:"transparent",color:C.muted,fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit",transition:"all 0.15s",display:"flex",alignItems:"center",gap:4}} onMouseEnter={e=>{e.currentTarget.style.borderColor=C.blue;e.currentTarget.style.color=C.blue;}} onMouseLeave={e=>{e.currentTarget.style.borderColor=C.border;e.currentTarget.style.color=C.muted;}}>✉️ Contact</button>
          {isStudent&&<div style={{background:C.violetSoft,border:"1px solid rgba(155,127,232,0.3)",borderRadius:20,padding:"3px 9px",fontSize:11,color:C.violet,fontWeight:800}}>🎓 STUDENT</div>}
          {isPro&&!isStudent&&<div style={{background:C.accentSoft,border:"1px solid rgba(121,186,236,0.3)",borderRadius:20,padding:"3px 9px",fontSize:11,color:C.blue,fontWeight:800}}>PRO</div>}
          {!isPro&&(<div style={{display:"flex",gap:5}}>
            <button onClick={()=>{setTPlan("pro");setTMode("essay");}} style={{background:C.accentSoft,border:"1px solid rgba(121,186,236,0.28)",borderRadius:20,padding:"3px 8px",fontSize:11,color:C.blue,fontWeight:800,cursor:"pointer",fontFamily:"inherit"}}>Pro $7↗</button>
            <button onClick={()=>{setTPlan("student");setTMode("humanize");}} style={{background:C.violetSoft,border:"1px solid rgba(155,127,232,0.28)",borderRadius:20,padding:"3px 8px",fontSize:11,color:C.violet,fontWeight:800,cursor:"pointer",fontFamily:"inherit"}}>Student $15↗</button>
          </div>)}
          <div style={{width:28,height:28,borderRadius:"50%",background:`linear-gradient(135deg,${planColor},${C.accent})`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,cursor:"pointer"}} title={user.email}>{user.avatar}</div>
        </div>
      </div>
      <div style={{display:"flex",borderBottom:`1px solid ${C.border}`,overflowX:"auto",scrollbarWidth:"none"}}>
        {MODES.map(m=>{
          const isActive=mode===m.id&&canAccess(m);
          const isStudentOnly=m.id==="humanize"||m.id==="academic";
          const tabColor=isStudentOnly?C.violet:planColor;
          return(<button key={m.id} onClick={()=>handleTab(m)} style={{flex:1,minWidth:54,background:isStudentOnly&&!isStudent?"rgba(155,127,232,0.05)":"transparent",border:"none",borderBottom:`2px solid ${isActive?tabColor:"transparent"}`,padding:"9px 2px 7px",color:isActive?tabColor:isStudentOnly?C.violet+"99":C.muted,fontFamily:"inherit",fontSize:11,cursor:"pointer",transition:"all 0.15s",display:"flex",flexDirection:"column",alignItems:"center",gap:2}}>
            <span style={{fontSize:11}}>{m.icon}</span>
            <span style={{fontSize:11,letterSpacing:"0.02em",whiteSpace:"nowrap"}}>{m.label}</span>
            {badgeFor(m)}
          </button>);
        })}
      </div>
      <div style={{maxWidth:580,margin:"0 auto",padding:"15px 13px 0"}}>
        {isPro&&(<div style={{background:isStudent?C.violetSoft:C.accentSoft,border:`1px solid ${isStudent?"rgba(155,127,232,0.2)":"rgba(121,186,236,0.2)"}`,borderRadius:7,padding:"8px 12px",marginBottom:16,display:"flex",alignItems:"center",gap:8,fontSize:11,color:isStudent?C.violet:"#a8d4f5"}}><span>{isStudent?"🎓":"✨"}</span><span>You're on the <strong style={{color:planColor}}>{isStudent?"Student plan":"Pro plan"}</strong> — all features unlocked!</span></div>)}
        ...
        <div style={{marginTop:44,paddingTop:16,borderTop:`1px solid ${C.border}`,display:"flex",justifyContent:"space-between",flexWrap:"wrap",gap:6,fontSize:11,color:"#162030",alignItems:"center"}}>
          <span>👻 GhostwriterMe · 9 AI Modes</span>
          <button onClick={()=>setShowContact(true)} style={{background:"transparent",border:"none",color:C.muted,fontSize:11,cursor:"pointer",fontFamily:"inherit",padding:0,textDecoration:"underline"}}>Contact Us ✉️</button>
...
export default function GhostwriterMeApp(){
  const [screen,setScreen]=useState("auth");const [user,setUser]=useState(null);const [billing,setBilling]=useState("monthly");const [tPlan,setTPlan]=useState("pro");const [showContact,setShowContact]=useState(false);
