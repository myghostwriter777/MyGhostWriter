  ...
  return {
    ...
    slides:(deck?.slides||[]).slice(0,guides.length||undefined).map((slide,index)=>{
      ...
      return {
        ...
        supportingText:clipWords(slide?.supportingText||"",44),
        bullets:(slide?.bullets||[]).map(item=>clipWords(item,44)).filter(Boolean).slice(0,3),
        narrativeRole:guide.role||slide?.narrativeRole||"detail",
        isHumorBeat:false,
        visualType,
        layout,
        visualLabel:clipWords(slide?.visualLabel||slide?.dataValue||"",8),
        dataValue:clipWords(slide?.dataValue||"",24),
        dataLabel:clipWords(slide?.dataLabel||"",32),
